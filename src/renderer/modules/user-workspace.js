...
function _workspaceSpaceDisplayName(space) {
...
function _chipLabel(target) {
  ...
  return sp ? (_workspaceSpaceDisplayName(sp) || sid) : sid;
}
...
function _updateChipForTarget(target) {
  ...
  if (prefix) prefix.textContent = t('workspace.chip_label', '工作空间：');
...
function _showWorkspaceDropdown(anchor, target) {
  ...
  void _loadSpaces().then(() => {
    ...
    if (existing) {
      if (typeof existing.__closeWorkspaceMenu === 'function') existing.__closeWorkspaceMenu();
...
function _renderSpaceMenu(anchor, target) {
  ...
  const renderList = (q) => {
    ...
    const spaces = _spacesCache
      .filter((s) => !query || _workspaceSpaceDisplayName(s).toLowerCase().includes(query))
      .sort((a, b) => String(b.last_conversation_at || b.updated_at || '')
        .localeCompare(String(a.last_conversation_at || a.updated_at || ''))
        || _workspaceSpaceDisplayName(a).localeCompare(_workspaceSpaceDisplayName(b), typeof getLang === 'function' && getLang() === 'en' ? 'en' : 'zh'));
    for (const s of spaces) rows.push(_spaceMenuItemHtml(_workspaceSpaceDisplayName(s), s.space_id, currentSid));
...
if (typeof window !== 'undefined') {
  ...
  if (typeof window.addEventListener === 'function') window.addEventListener('i18n-change', () => {
    ...
    if (!menu) return;
    const target = menu.dataset.wsTarget || 'conversation';
    const query = menu.querySelector('.workspace-menu-search')?.value || '';
    const wasFocused = menu.contains(document.activeElement);
    const anchor = document.querySelector(`.workspace-chip[data-ws-target="${target}"]`);
    if (typeof menu.__closeWorkspaceMenu === 'function') menu.__closeWorkspaceMenu();
    else menu.remove();
    if (!anchor) return;
    _renderSpaceMenu(anchor, target);
    const search = document.querySelector('#workspace-menu .workspace-menu-search');
    if (search) {
      search.value = query;
      search.dispatchEvent(new Event('input'));
      if (wasFocused) search.focus();
    }
  });
}
