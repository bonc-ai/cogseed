...
(function () {
  ...
  function _buildField(field, ctx, opts) {
    ...
    if (field.type === 'text' || field.type === 'number') {
      ...
      labelEl.appendChild(box);
      const text = document.createElement('span');
      text.dataset.i18n = 'chat.form.boolean_on';
      ...
      changePill.dataset.i18n = 'input.dir.change';
      ...
      const renderPath = () => {
        if (value) {
          ...
          pathLabel.dataset.i18n = 'input.dir.pick';
      ...
      note.dataset.formI18nTemplate = 'chat.form.unknown_type';
      note.dataset.formI18nType = String(field.type);
  ...
  function _buildFileWidget(field, ctx, opts) {
    ...
    picker.dataset.i18n = multiple ? 'chat.form.file_pick_multi' : 'chat.form.file_pick';
    picker.textContent = t(picker.dataset.i18n);
    ...
    function _renderChips() {
      ...
      for (const ent of entries) {
        ...
        if (ent.status === 'uploading') {
          ...
          tag.dataset.i18n = 'chat.form.file_uploading';
          tag.textContent = t('chat.form.file_uploading');
          chip.appendChild(tag);
        } else if (ent.status === 'failed') {
          const tag = document.createElement('span');
          tag.className = 'form-field-file-chip-tag';
          tag.dataset.i18n = 'chat.form.file_failed';
  ...
  function _validate(field, value) {
    if (field.required) {
      if (field.type === 'text' || field.type === 'textarea') {
        if (!String(value || '').trim()) return { key: 'chat.form.required_text' };
      } else if (field.type === 'multiselect') {
        if (!Array.isArray(value) || value.length === 0) return { key: 'chat.form.required_multiselect' };
      } else if (field.type === 'select') {
        if (!value) return { key: 'chat.form.required_select' };
      } else if (field.type === 'file') {
        const empty = field.multiple
          ? !Array.isArray(value) || value.length === 0
          : !value;
        if (empty) return { key: 'chat.form.required_file' };
      } else if (field.type === 'directory') {
        if (!String(value || '').trim()) return { key: 'chat.form.required_directory' };
      }
    }
    if (field.type === 'number' && typeof value === 'number') {
      if (typeof field.min === 'number' && value < field.min) return { key: 'chat.form.num_min', params: { min: field.min } };
      if (typeof field.max === 'number' && value > field.max) return { key: 'chat.form.num_max', params: { max: field.max } };
    }
    return null;
  }
  ...
  function renderChatInputForm(container, message, opts = {}) {
    ...
    title.dataset.i18n = submitted ? 'chat.form.readonly_title' : 'chat.form.title';
    ...
    if (submitted) {
      ...
      stamp.dataset.formSubmittedAt = form.submitted_at || '';
      const at = _formatFormDate(form.submitted_at);
    ...
    submitBtn.dataset.i18n = 'chat.form.submit';
    ...
    function _refreshSubmitState() {
      ...
      if (!submitBtn.dataset.locked) {
        ...
        if (allReady) delete submitBtn.dataset.i18nTitle;
    ...
    submitBtn.addEventListener('click', () => {
      ...
      if (errors.length) {
        ...
        errEl.textContent = errors
          .map((e) => t('chat.form.errors_prefix', {
            label: e.label || e.id,
            msg: t(e.key, e.params || {}),
          }))
          .join(' · ');
        return;
      }
      ...
      try {
        ...
        if (err && err.message) {
          ...
          errEl.__formI18nMessage = { key: 'chat.form.submit_failed' };
          errEl.__formValidationErrors = null;
  ...
  function _refreshChatFormsForLanguage() {
    document.querySelectorAll('.chat-input-form').forEach((container) => {
      if (typeof applyDomI18n === 'function') applyDomI18n(container);
      container.querySelectorAll('[data-form-i18n-template]').forEach((el) => {
        el.textContent = t(el.dataset.formI18nTemplate, { type: el.dataset.formI18nType || '' });
      });
      container.querySelectorAll('[data-form-submitted-at]').forEach((stamp) => {
        const at = _formatFormDate(stamp.dataset.formSubmittedAt);
        stamp.textContent = at
          ? t('chat.form.submitted_at', { time: at })
          : t('chat.form.submitted_no_time');
      });
      const errEl = container.querySelector('.form-error');
      if (errEl?.__formValidationErrors) {
        errEl.textContent = errEl.__formValidationErrors.map((error) => t('chat.form.errors_prefix', {
          label: error.label || error.id,
          msg: t(error.key, error.params || {}),
        })).join(' · ');
      } else if (errEl?.__formI18nMessage) {
        errEl.textContent = t(errEl.__formI18nMessage.key, errEl.__formI18nMessage.params || {});
      }
    });
  }
  ...
  if (typeof window.addEventListener === 'function') {
    window.addEventListener('i18n-change', _refreshChatFormsForLanguage);
  }
})();
