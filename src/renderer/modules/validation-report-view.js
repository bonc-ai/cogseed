...
function showValidationReport({ title, titleKey, titleParams, report, okLabel, okLabelKey, forceLabel, forceLabelKey } = {}) {
  return new Promise((resolve) => {
    ...
    const onI18nChange = () => {
      const titleEl = overlay.querySelector('#quality-report-title');
      if (titleEl && titleKey) titleEl.textContent = t(titleKey, titleParams || {});
      const body = overlay.querySelector('.quality-report-body');
      if (body) {
