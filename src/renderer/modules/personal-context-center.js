  ...
  async function action(actionName, dataset) {
    await withBusy(async () => {
      ...
      if (actionName === 'resources.save') {
        ...
        state.dashboard = result.dashboard;
        state.notice = { kind: 'info', key: 'personal_context_center.scope_saved', fallback: '接入范围已保存' };
        return;
      }
      ...
      if (actionName === 'briefing.test_delivery') {
        ...
        state.notice = result.result && result.result.ok
          ? { kind: 'info', key: 'personal_context_center.briefing_delivery_ok', fallback: '简报已投递' }
          : result.result && result.result.error
            ? { kind: 'error', text: result.result.error }
            : { kind: 'error', key: 'personal_context_center.briefing_delivery_failed', fallback: '简报投递失败' };
        return;
      }
      if (actionName === 'briefing.schedule') {
        ...
        state.notice = result.taskId
          ? { kind: 'info', key: 'personal_context_center.briefing_schedule_ok', fallback: '每日简报已设置' }
          : result.error
            ? { kind: 'error', text: result.error }
            : { kind: 'error', key: 'personal_context_center.briefing_schedule_failed', fallback: '每日简报设置失败' };
      }
    });
  }
