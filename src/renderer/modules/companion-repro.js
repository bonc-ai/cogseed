(function () {
  ...
  function _captureMountedUi(host) {
    const values = [];
    host.querySelectorAll('[data-companion-repro-field], [data-companion-repro-revision]').forEach((el) => {
      values.push({
        kind: el.dataset.companionReproField ? 'field' : 'revision',
        key: el.dataset.companionReproField || el.dataset.companionReproRevision,
        value: el.value,
      });
    });
    const active = document.activeElement;
    const activeKind = active?.dataset?.companionReproField ? 'field'
      : (active?.dataset?.companionReproRevision ? 'revision' : '');
    return {
      values,
      advancedOpen: !!host.querySelector('.companion-repro-advanced')?.open,
      activeKind,
      activeKey: active?.dataset?.companionReproField || active?.dataset?.companionReproRevision || '',
      selectionStart: active?.selectionStart ?? 0,
      selectionEnd: active?.selectionEnd ?? 0,
    };
  }

  function _renderMountedForLanguage() {
    const host = document.getElementById('companion-repro-host');
    if (!host || !_mountedCid || !host.childElementCount) return;
    const snapshot = _captureMountedUi(host);
    host.innerHTML = renderState(_mountedState);
    for (const item of snapshot.values) {
      const attr = item.kind === 'field' ? 'data-companion-repro-field' : 'data-companion-repro-revision';
      const el = host.querySelector(`[${attr}="${CSS.escape(item.key)}"]`);
      if (el) el.value = item.value;
    }
    const advanced = host.querySelector('.companion-repro-advanced');
    if (advanced) advanced.open = snapshot.advancedOpen;
    if (snapshot.activeKind && snapshot.activeKey) {
      const attr = snapshot.activeKind === 'field' ? 'data-companion-repro-field' : 'data-companion-repro-revision';
      const active = host.querySelector(`[${attr}="${CSS.escape(snapshot.activeKey)}"]`);
      if (active) {
        active.focus();
  ...
  if (typeof window.addEventListener === 'function') {
    window.addEventListener('i18n-change', _renderMountedForLanguage);
  }
}());
