...
function buildAsideBodyHtml(turns, opts = {}) {
  ...
  const pendingAnswer = opts.pendingErrorText != null
    ? `${t('aside.failed')}: ${opts.pendingErrorText}`
    : opts.pendingAnswer;
...
async function loadChatAside(cid, projectId) {
  ...
  _asideTurns = [];
  _asidePending = {};
  if (!_asideCid) { _renderAsideBody([], _asidePending); return; }
...
async function _submitAside() {
  ...
  input.value = '';
  let answer = '';
  _asidePending = { pendingQuestion: question, pendingAnswer: '' };
  _renderAsideBody(_asideTurns, _asidePending);
  ...
  const call = window.cogseed.stream('aside.askStream', {
    ...
    if (event.type === 'delta' && event.text) {
      answer += event.text;
      _asidePending = { pendingQuestion: question, pendingAnswer: answer };
      _renderAsideBody(_asideTurns, _asidePending);
...
async function _clearAside() {
  ...
  _asideTurns = [];
  _asidePending = {};
  _renderAsideBody([], _asidePending);
}
