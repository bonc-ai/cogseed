...
function renderSkillsCognitionCandidates() {
  ...
  host.innerHTML = `${categoryChips}<div class="skills-cognition-record-list">${items.map((c) => {
    ...
    // 「确认并落到本体」：统一候选池的第二个出口。promote 只把候选变成正式资产，
    // 这一条同时把资产语句写进选定的个人本体分组——本体是资产的一个去向，
    // 不是另一套候选生命周期，所以出口挂在同一张候选卡上，不另起界面。
    // 没有分组时不渲染：给一个点了必然失败的按钮，比不给更糟。
    const routeGroups = Array.isArray(_skillsCognitionState.ontologyGroups) ? _skillsCognitionState.ontologyGroups : [];
    const canRoute = actions.includes('promote') && routeGroups.length > 0;
    const routeRow = canRoute
      ? `<div class="skills-cognition-actions recall-candidate-route"><select data-recall-route-group="${escapeHtml(candidate.id)}">${routeGroups.map((group) => `<option value="${escapeHtml(group.group_id)}">${escapeHtml(group.title || group.group_id)}</option>`).join('')}</select><button class="btn btn-sm" data-recall-candidate-action="route-ontology" data-recall-candidate-id="${escapeHtml(candidate.id)}">${escapeHtml(_cognitionText('cognition.route_ontology', '确认并落到本体'))}</button></div>`
      : '';
