...
function _kbPickerRenderMessage() {
  const msg = document.getElementById('kb-picker-msg');
  if (!msg) return;
  msg.textContent = _kbPickerMessage
    ? t(_kbPickerMessage.key, _kbPickerMessage.params || {})
    : '';
  msg.className = _kbPickerMessage?.error ? 'form-msg err' : 'form-msg';
}

function _kbPickerSetMessage(key, params, error = true) {
  _kbPickerMessage = key ? { key, params, error } : null;
  _kbPickerRenderMessage();
}
...
async function confirmKbPicker() {
  ...
  if (!rawName) {
    _kbPickerSetMessage('kb_picker.name_needed');
    return;
  }
  if (rawName.includes('/') || rawName.includes('\\') || rawName.includes('..')) {
    _kbPickerSetMessage('kb_picker.name_bad_chars');
    return;
  }
  let name = rawName;
  ...
  if (_kbNameWeight(name) > _KB_NAME_MAX_WEIGHT) {
    _kbPickerSetMessage('kb_picker.name_too_long', { name });
    return;
  }
...
if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    ...
    if (_kbPickerTitleKey) {
      document.getElementById('kb-picker-title').textContent = t(_kbPickerTitleKey);
    }
    _kbPickerRenderTree();
    _kbPickerRenderTarget();
    _kbPickerRenderMessage();
