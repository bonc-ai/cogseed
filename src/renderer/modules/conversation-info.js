...
const ConversationInfo = (() => {
  ...
  function _renderCollaborationOverview() {
    ...
    if (!hasTask && !hasAgentActivity && !hasAttention && !mateState.loading && !mateState.error) {
    ...
    const mateHtml = (mateState.session || mateState.loading || mateState.error)
      ? _safeSection(() => _renderMateOverview(), loadFailed)
      : '';
    ...
    return `<div class="conversation-info-collaboration">${header}${mateHtml}${taskHtml}${agentHtml}${attentionHtml}</div>`;
  }
  ...
  function _renderRunContext() {
    ...
    var mateState = _snapshot.mate || {};
    var hasCollab = !!(_snapshot.collaboration || mateState.session || mateState.loading);
