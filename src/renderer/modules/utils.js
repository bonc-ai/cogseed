...
/** Resolve a Space name only when the persisted record carries a system marker. */
function localizedSpaceName(space, translate) {
  if (!space) return '';
  const rawName = String(space.name || space.space_id || '');
  const key = typeof space.system_name_key === 'string' ? space.system_name_key : '';
  const validKey = /^(onboarding\.temporary_space|ws\.(scenario|role_template)\.[a-z0-9_]+\.name)$/.test(key);
  const resolve = (i18nKey) => {
    if (typeof translate !== 'function') return '';
    try {
      const value = translate(i18nKey);
      return value && value !== i18nKey ? String(value) : '';
    } catch (_) { return ''; }
  };
  if (validKey) return resolve(key) || rawName;
  return rawName;
}
  ...
  const api = {
    ...
    setOptions(options, { value, placeholder } = {}) {
      ...
      if (typeof placeholder === 'string') {
        ...
        state.usesDefaultPlaceholder = false;
      }
...
if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    for (const api of _aiSelectInstances) {
      if (!api.el?.isConnected) {
        api.close();
        _aiSelectInstances.delete(api);
        continue;
      }
      api.refreshI18n();
    }
