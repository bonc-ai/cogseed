...
const _tveActiveStates = new Set();
...
if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    for (const state of _tveActiveStates) {
      if (state.destroyed) continue;
      const textarea = state.bodyEl.querySelector('[data-tve-textarea]');
      ...
      _tveRender(state);
      const nextTextarea = state.bodyEl.querySelector('[data-tve-textarea]');
      if (nextTextarea) {
