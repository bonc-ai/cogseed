...
async function openChatMdDrawer({ source, initialMode, title }) {
  ...
  _cmdState = { controller: null, source, title: resolvedTitle, usesDefaultTitle: !title, dirty: false };
...
if (typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    if (!_cmdState?.usesDefaultTitle) return;
    const title = _defaultTitle(_cmdState.source);
    _cmdState.title = title;
    const titleEl = document.getElementById('chat-md-drawer-title');
