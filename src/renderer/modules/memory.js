...
// scope list above. 这个面板只管**普通**记忆分组：角色模板分组由个人本体面板
// 负责（见 _memRenderGroupsSection 的 template_id 过滤），这里不解析模板文件
// 格式、不做字段/流水编辑。
let _memGroups = [];
let _memGroupsLoaded = false;
// At most one group's content editor open at a time.
//   { groupId, content, loaded } — null when closed.
 ...
 *  模板分组不出现在这个列表里（_memRenderGroupsSection 过滤 template_id），
 *  它们的表单/流水编辑在个人本体面板，走 PO 的复合 ref 通道。 */
