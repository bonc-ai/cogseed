...
function _conversationSpaceDisplayName(space) {
  return typeof localizedSpaceName === 'function'
    ? localizedSpaceName(space, t)
    : ((space && (space.name || space.space_id)) || '');
}
...
window.addEventListener('i18n-change', _refreshEmptyStateGreeting);
...
function _renderWelcomeCarryHtml(carry, resumeJson) {
  ...
  const summary = carry.map((c) => {
    const labelKey = kindKeys[c && c.kind];
    const label = labelKey ? t(labelKey) : String((c && c.label) || '');
    return t('chat.welcome_carry_item', { label, count: Number(c && c.count) || 0 });
  }).join(' · ');
...
async function _openConversationSpacePicker(cid) {
  ...
  const rows = spaceList.length
    ? spaceList.map((s) => {
      ...
      return `
        <button type="button" class="conversation-space-row" data-space-id="${escapeHtml(s.space_id)}" data-space-name="${escapeHtml(displayName)}" ${s.space_id === currentSpaceId ? 'data-current="1"' : ''}>
          <span class="conversation-space-mark">${escapeHtml((s.icon || (displayName || t('chat.conv_space_mark')).charAt(0)))}</span>
          <span class="conversation-space-name">${escapeHtml(displayName)}</span>
...
function _startSpaceRename(sid) {
  ...
  if (!sp) return;
  _spaceRenameId = sid;
  _spaceRenameValue = _conversationSpaceDisplayName(sp) || sid;
  renderConversationList();
}
...
async function _saveSpaceRename(sid, raw) {
  ...
  _spaceRenameId = '';
  renderConversationList();
  if (!name || name === _conversationSpaceDisplayName(_spaceById(sid))) return;
  try {
...
async function _deleteSpaceWithConfirm(sid) {
  const sp = _spaceById(sid);
  const name = sp ? (_conversationSpaceDisplayName(sp) || sid) : sid;
  let ok = false;
...
function _renderSpaceSidebarGroup(sp, convs) {
  ...
  const name = _conversationSpaceDisplayName(sp);
...
function _renderMarketplaceInstallCard(card, req, cid, msgId) {
  ...
  // 技能安装请求卡不再展示版本号（与技能库/市场页口径一致）；智能体保留。
  const version = (req.version && kind !== 'skill') ? t('marketplace.version').replace('{version}', String(req.version)) : '';
  const lang = getLang();
  ...
  try {
    window.cogseed.invoke('conversations.taskRefs.list', { cid }).then((res) => {
      try {
        ...
        const items = refs.map((r) => t('chat.taskref_inline_item', {
          kind: r.kind === 'asset' ? t('agent_picker.ref_asset', '资产') : t('agent_picker.ref_artifact', '产物'),
          name: r.name || '',
        })).join(t('common.list_separator'));
        strip.textContent = t('chat.taskref_inline_summary', { items });
...
function _renderSpaceAssetRefsHtml(refs) {
  ...
  return `<div class="chat-space-asset-refs"><span class="chat-space-asset-refs-label">${escapeHtml(t('chat.space_asset_refs_label', '引用资产'))}</span>${items.map((r) => {
    const name = (r && r.name) || '';
    if (!name) return '';
    const assetType = String((r && r.asset_type) || '');
    const label = assetType ? t('chat.space_asset_ref_with_type', { name, type: assetType }) : name;
    return `<span class="chat-space-asset-ref-chip" title="${escapeHtml(assetType || t('chat.space_asset_ref_title'))}">${escapeHtml(label)}</span>`;
  }).join('')}</div>`;
}
...
function _attachBubbleActions(msgDiv, getContent, opts = {}) {
  ...
  btn.addEventListener('click', async (e) => {
    ...
    const pick = await pickKbLocation({
      ...
      titleKey: 'chat.archive_picker_title',
      scope: targetScope,
    });
    if (!pick) return;
    btn.disabled = true;
...
async function _maybeApplyCliFallback(cid, opts = {}) {
  ...
  if (typeof uiToast === 'function') {
    uiToast(t('cli_fallback.no_api_key', { label }), { variant: 'warning', timeoutMs: 6000 });
  }
  return true;
}
...
async function _maybeAutoCliFallbackOnModelFailure(cid, ev) {
  ...
  if (ok && typeof uiToast === 'function') {
    uiToast(t('cli_fallback.model_failed', {}), { variant: 'warning', timeoutMs: 6000 });
  }
}
...
async function _maybeAutoSwitchCliOnFailure(cid, ev) {
  ...
  if (!ok) {
    ...
    if (typeof uiToast === 'function') {
      uiToast(t('cli_fallback.no_other_agent', { cli: failedCli }), { variant: 'warning', timeoutMs: 8000 });
    }
    return;
  }
  ...
  if (typeof uiToast === 'function') {
    ...
    uiToast(t('cli_fallback.switched', { label }), { variant: 'warning', timeoutMs: 6000 });
  }
}
...
async function _cliFallbackGuideUser() {
  ...
  if (desktopApps.includes('Claude')) {
    uiToast(
      t('chat.cli_fallback_claude_desktop'),
      { variant: 'warning', timeoutMs: 10000 },
    );
    return;
  }
  uiToast(
    t('chat.cli_fallback_unavailable'),
    { variant: 'warning', timeoutMs: 10000 },
  );
}
...
// with a newly resumed agent turn. The sole legacy exception is a real
