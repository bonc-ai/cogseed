...
function _iclUpdateLinks(session) {
  ...
  for (const url of urls) {
    ...
    btn.innerHTML = `${_iclIcon('external', 'interactive-cli-link-icon')}<span data-i18n="interactive_cli.open_link">${_iclEsc(_iclT('interactive_cli.open_link', 'Open link'))}</span>`;
    btn.title = url;
  ...
  card.innerHTML = `
    <div class="interactive-cli-head">
      ...
      <div class="interactive-cli-actions">
        <button type="button" class="interactive-cli-stop" data-icl-stop data-i18n-title="interactive_cli.stop" data-i18n-aria-label="interactive_cli.stop" title="${_iclEsc(_iclT('interactive_cli.stop', 'Stop'))}" aria-label="${_iclEsc(_iclT('interactive_cli.stop', 'Stop'))}">
          ${_iclIcon('squareFilled', 'interactive-cli-stop-icon')}
        </button>
        <button type="button" class="interactive-cli-close" data-icl-close data-i18n-title="common.close" data-i18n-aria-label="common.close" title="${_iclEsc(_iclT('common.close', 'Close'))}" aria-label="${_iclEsc(_iclT('common.close', 'Close'))}">
    ...
    <form class="interactive-cli-form" data-icl-form>
      <input class="interactive-cli-input" data-icl-input data-i18n-placeholder="interactive_cli.input_placeholder" type="text" autocomplete="off" spellcheck="false" placeholder="${_iclEsc(_iclT('interactive_cli.input_placeholder', 'Input for the CLI'))}" />
      <label class="interactive-cli-sensitive">
        <input type="checkbox" data-icl-sensitive />
        <span data-i18n="interactive_cli.sensitive">${_iclEsc(_iclT('interactive_cli.sensitive', 'Sensitive'))}</span>
      </label>
      <button type="submit" class="btn btn-primary btn-sm interactive-cli-send" data-icl-send data-i18n="interactive_cli.send">${_iclEsc(_iclT('interactive_cli.send', 'Send'))}</button>
    </form>
  `;
...
if (typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    for (const session of _interactiveCliSessions.values()) {
      ...
      _iclApplyState(session);
      if (typeof applyDomI18n === 'function') applyDomI18n(session.card);
    }
  });
}
