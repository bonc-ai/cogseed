...
(function () {
  ...
  function noticeText(notice) {
    if (!notice) return '';
    return notice.key ? tr(notice.key, notice.fallback || '') : String(notice.text || '');
  }
  ...
  function renderReview(state) {
    ...
    for (const item of items) {
      ...
      const refs = Array.isArray(item.sourceRefs) ? item.sourceRefs.filter(Boolean) : [];
      const locale = typeof getLang === 'function' && getLang() === 'en' ? 'en-US' : 'zh-CN';
      const source = refs.length
        ? new Intl.ListFormat(locale, { style: 'long', type: 'conjunction' }).format(refs.map(String))
        : tr('personal_context_center.source_unknown', '未知来源');
      card.append(el('p', '', tr('personal_context_center.source_value', '来源：{value}').replace('{value}', source)));
  ...
  function render(state) {
    ...
    if (state.notice) root.append(el('div', `messaging-notice is-${state.notice.kind || 'info'}`, noticeText(state.notice)));
    return root;
  }
