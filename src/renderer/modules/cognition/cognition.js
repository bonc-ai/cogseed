...
(function () {
  ...
  async function addEvidence(asset) {
    ...
    if (!summary) return;
    const sourceLabel = (await uiPrompt(
      translate('cognition.prompt.evidence_source', '请输入证据来源'),
      translate('cognition.capture.default_source', '当前会话'),
    ))?.trim();
  ...
  async function recordReuse(asset) {
    if (typeof uiPrompt !== 'function') return;
    const sourceLabel = (await uiPrompt(
      translate('cognition.prompt.reuse_source', '请输入本次复用场景'),
      translate('cognition.capture.default_source', '当前会话'),
    ))?.trim();
    if (!sourceLabel) return;
