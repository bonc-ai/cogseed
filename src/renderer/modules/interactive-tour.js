...
const TOUR_STEPS = [
  {
    id: 'welcome-continue',
    titleKey: 'tour.welcome_continue.title',
    descriptionKey: 'tour.welcome_continue.description',
    title: '带着这些继续',
  ...
  {
    id: 'recall',
    titleKey: 'tour.recall.title',
    descriptionKey: 'tour.recall.description',
    title: '了解其他核心部分',
  ...
  },
  {
    ...
    id: 'recall-review',
    titleKey: 'tour.recall_review.title',
    descriptionKey: 'tour.recall_review.description',
    title: '候选认知，你说了算',
  ...
  {
    id: 'recall-assets',
    titleKey: 'tour.recall_assets.title',
    descriptionKey: 'tour.recall_assets.description',
    title: '确认后，认知归你',
  ...
  },
  {
    ...
    titleKey: 'tour.recall_asset_detail.title',
    descriptionKey: 'tour.recall_asset_detail.description',
    title: '一条资产里有什么',
...
function _tourText(key, fallback) {
...
function _showTourStep(stepIndex) {
  ...
  const nextBtnHtml = step.requireAction
    ...
    : stepIndex < TOUR_STEPS.length - 1
      ? `<button type="button" class="tour-tooltip-btn primary" data-tour-action="next" data-i18n="tour.got_it">${_tourEsc(_tourText('tour.got_it', '知道了'))}</button>`
      : `<button type="button" class="tour-tooltip-btn primary" data-tour-action="next" data-i18n="tour.finish">${_tourEsc(_tourText('tour.finish', '完成'))}</button>`;
  const tooltipHTML = `
    <div class="tour-tooltip-content">
      <div class="tour-tooltip-title" data-i18n="${_tourEsc(step.titleKey)}">${_tourEsc(_tourText(step.titleKey, step.title))}</div>
      <div class="tour-tooltip-desc" data-i18n="${_tourEsc(step.descriptionKey)}">${_tourEsc(_tourText(step.descriptionKey, step.description))}</div>
      <div class="tour-tooltip-actions">
        <button type="button" class="tour-tooltip-link" data-tour-action="skip" data-i18n="tour.skip">${_tourEsc(_tourText('tour.skip', '跳过'))}</button>
        ${nextBtnHtml}
      </div>
    </div>
...
function _showTourFinishCard() {
  if (_tourFinishTimer) clearTimeout(_tourFinishTimer);
  _tourTooltip.classList.remove('left', 'right', 'top', 'bottom');
  _tourTooltip.classList.add('tour-tooltip-finish');
  _tourTooltip.innerHTML = `
    <div class="tour-tooltip-content">
      <div class="tour-tooltip-title" data-i18n="tour.complete.title">${_tourEsc(_tourText('tour.complete.title', '引导完成'))}</div>
      <p class="tour-tooltip-desc" data-i18n="tour.complete.description">${_tourEsc(_tourText('tour.complete.description', '你已把工作空间的能力带入任务开始执行。认知资产里沉淀的能力会随任务自动跟上。'))}</p>
      <div class="tour-tooltip-actions">
        <button type="button" class="tour-tooltip-btn primary" data-tour-action="done" data-i18n="tour.got_it">${_tourEsc(_tourText('tour.got_it', '知道了'))}</button>
      </div>
    </div>
  `;
...
window.addEventListener('i18n-change', () => {
  if (!_tourState || !_tourTooltip) return;
  if (_tourState.completed) _showTourFinishCard();
  else _showTourStep(_tourState.currentStep);
});
