...
function _ensureGuardBanner() {
  ...
  const warningIcon = (typeof window !== 'undefined' && typeof window.uiIconHtml === 'function')
    ? window.uiIconHtml('warning', 'ui-icon')
    : '';
  const closeIcon = (typeof window !== 'undefined' && typeof window.uiIconHtml === 'function')
    ? window.uiIconHtml('x', 'ui-icon')
    : '×';
  el.innerHTML = `
    <button type="button" class="model-guard-dismiss" title="${escapeHtml(t('model_guard.dismiss'))}" aria-label="${escapeHtml(t('model_guard.dismiss'))}">${closeIcon}</button>
    <span class="model-guard-icon" aria-hidden="true">${warningIcon}</span>
    <span class="model-guard-copy">
      <strong class="model-guard-title">${escapeHtml(t('model_guard.title'))}</strong>
      <span class="model-guard-text">${escapeHtml(t('model_guard.description'))}</span>
    </span>
  ...
  window.addEventListener('i18n-change', () => {
    ...
    const title = _guardBannerEl.querySelector('.model-guard-title');
    const txt = _guardBannerEl.querySelector('.model-guard-text');
    const cta = _guardBannerEl.querySelector('.model-guard-cta');
    const dismiss = _guardBannerEl.querySelector('.model-guard-dismiss');
    if (title) title.textContent = t('model_guard.title');
    if (txt) txt.textContent = t('model_guard.description');
    if (cta) cta.textContent = t('model_guard.cta');
    if (dismiss) {
      const label = t('model_guard.dismiss');
      dismiss.title = label;
...
function _applyGuardVisuals() {
  ...
  // CLI-based external Agents do not replace this application's own model
  // configuration. Keep the reminder visible until an API credential or a
  // complete CC Switch credential has been configured.
  if (_hasConfiguredModel) _guardDismissed = false;
  if (banner) banner.style.display = _hasConfiguredModel || _guardDismissed ? 'none' : '';
