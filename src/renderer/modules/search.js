...
if (typeof window !== 'undefined') {
  window.addEventListener('i18n-change', () => {
    const overlay = document.getElementById('search-overlay');
    if (!overlay || overlay.style.display === 'none') return;
    if (_searchLastError !== null) {
      _renderSearchError(_searchLastError);
      return;
    }
    const query = (document.getElementById('search-input')?.value || _searchLastQuery).trim();
    if (query && (_searchLastQuery || _searchResults.length)) _renderSearchResults(query);
