...
// `getUiLanguage` IPC pair remains as a fallback inside `initI18n()` for the
...
function getLocaleMeta(lang) {
  return _LOCALE_BY_CODE[lang] || _LOCALE_BY_CODE.zh;
}

function fallbackChain(lang) {
  const out = [];
  const seen = new Set();
  let cur = isSupportedLang(lang) ? lang : 'zh';
...
async function initI18n() {
  ...
  try {
    const langRes = await window.cogseed.getUiLanguage();
    const localesRes = await window.cogseed.getLocales();
    if (langRes && langRes.ok && isSupportedLang(langRes.uiLanguage)) {
      _currentLang = langRes.uiLanguage;
    }
    if (localesRes && localesRes.ok && localesRes.tables) {
...
async function setLang(lang) {
  ...
  try {
    ...
    const res = await window.cogseed.setUiLanguage(lang);
    if (res && res.ok && res.uiLanguage) {
      _currentLang = res.uiLanguage;
    } else {
      _currentLang = lang;
    }
  } catch (err) {
...
async function refreshLangFromMain() {
  try {
    const res = await window.cogseed.getUiLanguage();
    const next = res && res.ok && isSupportedLang(res.uiLanguage) ? res.uiLanguage : _currentLang;
