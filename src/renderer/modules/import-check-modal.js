    ...
    return new Promise((resolve) => {
      ...
      const stateDefs = {
        ...
        pass: {
          ...
          actions: { primary: { id: 'done', label: t('import_check.done') }, ghost: { id: 'view', label: t('import_check.view_skill') } },
        },
        risk: {
          cls: 'warn', ic: 'warn', title: () => t('import_check.risk_title'), score: true,
          desc: () => t('import_check.risk_desc'),
          actions: {
            primary: { id: 'keep', label: t('import_check.keep') },
            secondary: [{ id: 'recheck', label: t('import_check.recheck') }],
            danger: { id: 'delete', label: t('import_check.delete') },
          },
        },
        blocked: {
          cls: 'bad', ic: 'bad', title: () => t('import_check.blocked_title'), score: true,
          desc: () => t('import_check.blocked_desc'),
          actions: { primary: { id: 'close', label: t('import_check.got_it') }, ghost: { id: 'export', label: t('import_check.export_report') } },
        },
        unavailable: {
          cls: 'muted', ic: 'miss', title: () => t('import_check.unavailable_title'), score: null,
          desc: () => t('import_check.unavailable_desc'),
          actions: {
            primary: { id: 'draft', label: t('import_check.keep_draft') },
            secondary: [{ id: 'retry', label: t('import_check.retry') }],
          },
        },
      };
      ...
      function render() {
        ...
        if (def.actions) {
          const a = def.actions;
          foot += a.primary ? `<button type="button" class="btn imp-btn-primary" data-act="${a.primary.id}">${esc(a.primary.label)}</button>` : '';
          (a.secondary || []).forEach((s) => { foot += `<button type="button" class="btn" data-act="${s.id}">${esc(s.label)}</button>`; });
          foot += '<span class="imp-spacer"></span>';
          if (a.danger) foot += `<button type="button" class="btn imp-btn-danger" data-act="${a.danger.id}">${esc(a.danger.label)}</button>`;
          if (a.ghost) foot += `<button type="button" class="btn imp-btn-ghost" data-act="${a.ghost.id}">${esc(a.ghost.label)}</button>`;
        }
