...
const _mveActiveStates = new Set();
...
function _mveRefreshForLanguage(state) {
  if (!state || state.destroyed) return;
  const textarea = state.bodyEl.querySelector('[data-mve-textarea]');
  const activeBefore = document.activeElement;
  const hadTextareaFocus = activeBefore === textarea;
  const selectionStart = textarea?.selectionStart ?? 0;
  const selectionEnd = textarea?.selectionEnd ?? 0;
  const textareaScrollTop = textarea?.scrollTop ?? 0;
  const bodyScrollTop = state.bodyEl.scrollTop;
  if (textarea) state.draft = textarea.value;
  _mveRender(state);
  const nextTextarea = state.bodyEl.querySelector('[data-mve-textarea]');
  if (nextTextarea) {
    nextTextarea.scrollTop = textareaScrollTop;
    nextTextarea.setSelectionRange(selectionStart, selectionEnd);
  }
  state.bodyEl.scrollTop = bodyScrollTop;
  if (hadTextareaFocus) nextTextarea?.focus();
  else if (activeBefore?.isConnected && typeof activeBefore.focus === 'function') activeBefore.focus();
  else nextTextarea?.blur();
}
...
if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    for (const state of _mveActiveStates) _mveRefreshForLanguage(state);
  });
}
