...
async function _mpLoadOss(opts = {}) {
  ...
  try {
    ...
    if (_mpState && _mpState._ossLoadGen === myGen) {
    ...
    if (typeof isQualityRejectionError === 'function' && isQualityRejectionError(msg)) {
      ...
      if (report) {
        ...
        const action = await showValidationReport({
          titleKey: 'quality.install_rejected_title',
          titleParams: { name: rejectedName },
          title: t('quality.install_rejected_title', { name: rejectedName }),
          report,
          forceLabelKey: 'marketplace.override_confirm',
          forceLabel: t('marketplace.override_confirm'),
        });
        if (action === 'force') {
...
function _mpCompareMarketplaceName(a, b) {
  const byName = compareDisplayNames(a?.name || a?.id || '', b?.name || b?.id || '');
