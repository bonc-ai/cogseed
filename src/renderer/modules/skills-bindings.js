 ...
 * 候选决定失败时给用户看的话。
 *
 * 后端抛的 message 是内部契约语言（`recall candidate is terminal`、`candidate
 * does not meet the formal asset bar: ...`），过去被原样 `uiAlert` 出去——
 * `formal-assets/promotion.ts::describePromotionBlock` 自己的注释就写着那串是
 * 日志/回执用、**不是 UI 文案**。按 `result.code` 翻译（码由 candidate-
 * capabilities.ts::recallCandidateError 与 PromotionBlockedError 打上，IPC 分发器
 * 已透传到返回体）；晋升闸门再按 `promotionReasons` 逐条展开——只说"不够格"而不说
 * 缺什么，用户没法补。取不到已知码时退回原始 error，宁可露出英文也不吞掉失败。
 *
 * 与 `_recallProofErrorText` 同一套写法，不合并：两者的码表属于不同的业务链，
 * 合成一张表会让任何一边加码都要动另一边。
 */
...
function _recallCandidateErrorText(result) {
  const text = (key, fallback) => (typeof t === 'function' && t(key) !== key ? t(key) : fallback);
  const code = String((result && result.code) || '');
  if (code === 'promotion_blocked') {
    ...
    const lines = reasons
      ...
      .filter(Boolean)
      .map((entry) => `\u00b7 ${text(entry[0], entry[1])}`);
    const head = text('cognition.candidate_error_promotion_blocked', '这条候选还不够格成为正式资产：');
    return lines.length
      ? `${head}\n${lines.join('\n')}`
      : `${head}${text('cognition.candidate_error_promotion_blocked_unknown', '请检查内容、类型与作用范围。')}`;
  }
  const entry = RECALL_CANDIDATE_ERROR_TEXTS[code];
  if (entry) return text(entry[0], entry[1]);
  return (result && result.error) || text('cognition.candidate_error_generic', '这次操作没有完成，请稍后重试。');
}
...
function _initSkillsCognitionBindings() {
  ...
  const runRecallAssetAction = async (control, actionName, assetId) => {
    ...
    try {
      ...
      // 键直接写死在 _cognitionText 调用里，让静态 i18n 扫描能看到它们。
      const confirmations = {
        revoke: () => _cognitionText('cognition.asset_revoke_confirm', '确认从 Recall 中移除这条记忆？原始会话和文件不会被删除。'),
        delete: () => _cognitionText('cognition.asset_delete_confirm', '删除后进入保留期，期内可以恢复；保留期过后将无法找回。确认删除？'),
        purge: () => _cognitionText('cognition.asset_purge_confirm', '彻底清除会删掉这条认知的内容和全部历史版本，且无法恢复。确认清除？'),
      };
      ...
      if (typeof uiToast === 'function') {
        const done = {
          pause: () => _cognitionText('cognition.asset_action_pause_done', '已暂停使用'),
          resume: () => _cognitionText('cognition.asset_action_resume_done', '已恢复使用'),
          revoke: () => _cognitionText('cognition.asset_action_revoke_done', '已移除'),
          archive: () => _cognitionText('cognition.asset_action_archive_done', '已归档'),
          restore: () => _cognitionText('cognition.asset_action_restore_done', '已恢复'),
          delete: () => _cognitionText('cognition.asset_action_delete_done', '已删除，保留期内可恢复'),
          purge: () => _cognitionText('cognition.asset_action_purge_done', '已彻底清除'),
        };
        uiToast((done[actionName] || (() => '已完成'))(), { variant: 'success' });
      }
    } catch (error) {
      _skillsCognitionState.assetHistoryById ||= {};
      if (actionName === 'versions') {
  ...
  panel.addEventListener('click', async (event) => {
    ...
    // 「查看关于我」辅助入口：只展示个人本体本身（左栏导航 + 右栏内容），
    // 不带四类资产卡和资产工作台。进入 assets 页但隐藏资产工作台，只展开
    // 个人本体骨架；「返回认知树」回到树视图。
    const openOntology = event.target.closest('[data-cognition-open-ontology]');
    if (openOntology) {
    ...
    if (skillDecision) {
      ...
      const messageKey = decision === 'accept' ? _cognitionText('cognition.skillupdate_accept_confirm', '确认接受这次 Skill 升级？')
        : decision === 'defer' ? _cognitionText('cognition.skillupdate_defer_confirm', '暂缓这次升级，保留当前版本？')
        : _cognitionText('cognition.skillupdate_reject_confirm', '拒绝这次升级？');
      if (typeof uiConfirm === 'function' && !(await uiConfirm(messageKey))) return;
      ...
      try {
        ...
        if (typeof uiToast === 'function') uiToast(decision === 'accept' ? _cognitionText('cognition.skillupdate_accept_done', 'Skill 已升级')
          : decision === 'defer' ? _cognitionText('cognition.skillupdate_defer_done', '已暂缓升级')
          : _cognitionText('cognition.skillupdate_reject_done', '已拒绝本次升级'), { variant: 'success' });
    ...
    if (abilityAssetAction) {
      ...
      const promptLabel = action === 'acknowledge-recommendation'
        ? _cognitionText('cognition.asset_reason_acknowledge_prompt', '请输入确认建议的原因：')
        : action === 'pause'
          ? _cognitionText('cognition.asset_reason_pause_prompt', '请输入暂停原因：')
          : action === 'resume'
            ? _cognitionText('cognition.asset_reason_resume_prompt', '请输入恢复原因：')
            : _cognitionText('cognition.asset_reason_revoke_prompt', '请输入撤销原因：');
    ...
    // 打开来源选择面板。只切状态并重画，不取数——来源目录在
    // loadSkillsCognitionSnapshot 时已经随 recall.sources.list 一起加载好了。
    const evidenceAdd = event.target.closest('[data-recall-evidence-add]');
    if (evidenceAdd) {
      _skillsCognitionState.evidencePickerCandidateId = evidenceAdd.dataset.recallEvidenceAdd || '';
      renderSkillsCognitionCandidateDetail();
      return;
    }
    const evidencePickerClose = event.target.closest('[data-recall-evidence-picker-close]');
    if (evidencePickerClose) {
      _skillsCognitionState.evidencePickerCandidateId = '';
      renderSkillsCognitionCandidateDetail();
      return;
    }
    // 选中一条来源：把**目录里的原始 ref 对象**整条收下（含 title/subtype/scope），
    // 不是拿 kind:id 现拼一个——现拼的会丢掉元数据，且等于又一次"手造 ref"。
    const evidencePick = event.target.closest('[data-recall-evidence-pick]');
    if (evidencePick) {
      const key = String(evidencePick.dataset.recallEvidencePick || '');
      const candidateId = _skillsCognitionState.evidencePickerCandidateId;
      const groups = Array.isArray(_skillsCognitionState.sources) ? _skillsCognitionState.sources : [];
      const found = groups.flatMap((group) => (Array.isArray(group?.items) ? group.items : []))
        .find((item) => item && `${item.kind}:${item.id}` === key);
      if (found && candidateId) {
        const picked = _skillsCognitionState.evidencePicked;
        const refs = picked && picked.candidateId === candidateId && Array.isArray(picked.refs) ? picked.refs : [];
        if (!refs.some((ref) => `${ref.kind}:${ref.id}` === key)) {
          _skillsCognitionState.evidencePicked = { candidateId, refs: [...refs, found] };
        }
        renderSkillsCognitionCandidateDetail();
      }
      return;
    ...
    // 逐条移除证据：只动 DOM，不即时提交——用户可能连着删几条再一次「确认并限域」。
    // 保存时按界面上**剩下的** chip 收集，所以这里不需要额外状态。
    const evidenceRemove = event.target.closest('[data-recall-evidence-remove]');
    if (evidenceRemove) {
      const chip = evidenceRemove.closest('[data-recall-evidence-ref]');
    ...
    if (recallAction) {
      ...
      try {
        ...
        const card = (candidate?.suggestedType === 'personal'
          || actionName === 'save-and-promote' || actionName === 'save-only')
          ? (() => {
          ...
          : null;
        // 「确认并限域」提交的正是范围本身。范围留空时后端会把候选静默降回
        // weak_observation、再照常晋升出一条没有作用范围的资产——正是这一页右侧
        // 「没有范围的规则不会被带入任何任务」要拦住的事。所以空范围在这里就停，
        // 并且停在**高风险确认之前**：先让用户确认一次高风险、再告诉他范围没填，
        // 等于把同一次点击问两遍。
        const scopeField = actionName === 'save-and-promote'
          ? (card && typeof card.querySelector === 'function' ? card.querySelector('[data-recall-edit-scope]') : null)
          : null;
        if (scopeField && !String(scopeField.value || '').trim()) {
          if (typeof scopeField.focus === 'function') scopeField.focus();
          if (typeof uiAlert === 'function') {
            await uiAlert(_cognitionText(
              'cognition.candidate_scope_required',
              '先填写作用范围：没有范围的资产不会被带入任何任务。',
            ));
          }
          return;
        }
        // 类型可以在这一次提交里被改掉，而个人模板落点选择器是按**原**类型渲染的。
        // 把 personal 改成 rule 之后还把画像落点发出去，就是往一条已经不是个人
        // 本体的资产上绑画像字段。按这次真正提交的类型判。
        const submittedType = actionName === 'save-and-promote'
        ...
        // save-only 与 save-and-promote 提交同一份编辑，区别只在后面接不接晋升。
        // 证据不足的候选（canEdit 但 !canPromote）只有这一条路能把证据补进去——
        // 详情页此前没有任何保存入口，用户能改却存不下。
        if (actionName === 'save-and-promote' || actionName === 'save-only') {
          if (!card || !candidate) throw new Error(_recallCandidateErrorText({ code: 'recall_candidate_not_found' }));
          ...
          // 证据引用现在是**只读展示 + 逐条删除**：界面上留下哪几条，就提交哪几条。
          // 不再有自由输入，所以用户无法凭空造出一条 ref（那会直接满足 reviewReady
          // 与 canPromote 的证据判据）。原 ref 对象整体带回，title / scope /
          // sourceVersion / authorizationRef 一并保住。
          const refKey = (ref) => `${ref.kind}:${ref.id}`;
          // 已选未保存的来源也要认：它们来自 recall.sources.list 的原始条目，
          // 服务端 assertResolvableNewSourceRefs 会再验一次存在性。
          const pickedState = _skillsCognitionState.evidencePicked;
          const pickedRefs = pickedState && pickedState.candidateId === candidateId && Array.isArray(pickedState.refs)
            ? pickedState.refs : [];
          const knownRefs = new Map([...(candidate.evidenceRefs || []), ...(candidate.sourceRefs || []), ...pickedRefs]
            .filter((ref) => ref && ref.kind && ref.id)
            .map((ref) => [refKey(ref), ref]));
          // 容器可能没有 querySelectorAll（旧渲染路径/精简容器）。缺方法就当"这一屏
          // 没渲染证据区"，沿用候选原有引用——绝不能让取不到 DOM 变成"清空证据"，
          // 也不能因此抛异常把整次保存吞掉。
          const chips = typeof card.querySelectorAll === 'function'
            ? [...card.querySelectorAll('[data-recall-evidence-ref]')]
            : [];
          const evidenceEl = chips.length ? chips : null;
          const sourceRefs = chips.length
            ? chips.map((el) => knownRefs.get(String(el.dataset.recallEvidenceRef || ''))).filter(Boolean)
        ...
        const result = await window.cogseed.invoke(channel, payload);
        if (!result?.ok) throw new Error(_recallCandidateErrorText(result));
        if (actionName === 'save-and-promote') {
          ...
          if (!promoted?.ok) throw new Error(_recallCandidateErrorText(promoted));
        }
        _skillsCognitionState.editingRecallCandidateId = '';
        _skillsCognitionState.writingRecallCandidateId = '';
        // 已选证据随本次提交落库，选择态必须清掉——留着会让下一条候选带上
        // 上一条挑的来源，而且重取快照后 chip 会重复渲染一份。
        _skillsCognitionState.evidencePicked = null;
        _skillsCognitionState.evidencePickerCandidateId = '';
        await loadSkillsCognitionSnapshot();
        ...
        // 候选详情页是**单条**候选的整页决定面：这一条决定完，页面上每个字都过期了。
        // 而 loadSkillsCognitionSnapshot 只重画任务视图（inbox / sources / captures /
        // assets / governance），详情页不在它的名单里——不主动收尾的话，用户面对的是
        // 一张仍写着「待确认」的旧页，按钮还在且已被 finally 重新 enable，再点一次
        // 必然撞终态报错，那句成功 toast 就成了假话。回「待我处理」：历史带交给
        // 切页的 enter 自己拉，用户也被放回下一件事。
        if (_skillsCognitionState.page === 'candidate' && actionName !== 'save-only'
          && typeof switchSkillsCognitionPage === 'function') {
          switchSkillsCognitionPage('inbox');
        } else {
          if (typeof loadCognitionReviewHistory === 'function') await loadCognitionReviewHistory();
          // save-only 是"接着改"，不是"决定完了"：留在这一页重画——证据补齐后
          // 候选会迁出 weak_observation，「确认并限域」这时才长出来。
          if (_skillsCognitionState.page === 'candidate' && typeof _cognitionRenderCurrentPage === 'function') {
        ...
        // 失败后详情页同样要重画：候选状态可能已经在后端变了（终态、失败、被别处
        // 确认过），停在旧的一屏只会让用户继续点一个已经不成立的按钮。
        if (typeof _cognitionRenderCurrentPage === 'function') _cognitionRenderCurrentPage();
        if (typeof uiAlert === 'function') await uiAlert((error && error.message) || String(error));
      }
      finally {
        _skillsCognitionState.writingRecallCandidateId = '';
