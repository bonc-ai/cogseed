...
(function () {
  ...
  function shell(agentName) {
    const nav = pages.map(([id, key, iconName], index) => `
      <button type="button" class="ew-nav-item${index === 0 ? ' is-active' : ''}" data-ew-page="${id}"${index === 0 ? ' aria-current="page"' : ''}>
    ...
    return `<section class="expense-workbench" aria-label="${escape(text('title', '报销智能体'))}">
      ...
      <div class="ew-error" id="ew-error" role="alert" aria-live="assertive" hidden></div>
      <div class="ew-progress" id="ew-progress" role="status" aria-live="polite" hidden></div>
      <div class="ew-layout">
  ...
  function unconfigured() {
    return `<div class="ew-page" data-ew-unconfigured><section class="ew-panel ew-settings-panel"><div class="ew-empty"><strong>${escape(text('configure_title', '连接本地报销项目'))}</strong><span>${escape(text('configure_hint', '选择包含 .venv 的报销智能体目录，Mate 不会启动网页服务。'))}</span><button type="button" class="btn btn-primary" data-ew-configure>${escape(text('configure', '选择项目目录'))}</button></div></section></div>`;
  }

  function status(value) {
    const clean = String(value || 'collecting');
    const label = text(`status_${clean}`, clean);
    return `<span class="ew-status ew-status-${escape(clean.replace(/[^a-z0-9_-]/gi, '-'))}" data-ew-status="${escape(clean)}">${escape(label)}</span>`;
  }

  function actionState(state, key) {
    return state && state.busy && state.busy[key]
  ...
  function assistant(state) {
    ...
    return `<div class="ew-page ew-assistant-page">
      <div class="ew-page-intro"><div><span class="ew-kicker">${escape(text('nav_assistant', '报销助手'))}</span><h2>${escape(text('assistant_heading', '把材料、草稿与预审结论放在同一案件'))}</h2></div><button type="button" class="btn btn-primary" data-ew-create${actionState(state, 'create')}>${escape(text('create_application', '新建报销申请'))}</button></div>
  ...
  function applications(state) {
    ...
    const approval = selected && selected.approval ? selected.approval : {};
    ...
    const notifications = selected && Array.isArray(selected.feishu_notifications)
      ? selected.feishu_notifications : [];
    const canRetryFeishuNotifications = !!app
      && notifications.some((item) => item && (item.state === 'failed' || item.state === 'pending'));
    const approvalActions = approval.can_decide && approval.artifact_hash && Array.isArray(approval.pending_roles)
      ? approval.pending_roles.map((role) => `<span class="ew-approval-action"><strong>${escape(role)}</strong><button type="button" class="btn btn-secondary btn-sm" data-ew-approve="approve" data-ew-approval-role="${escape(role)}">${escape(text('approve', '批准'))}</button><button type="button" class="btn btn-ghost btn-sm" data-ew-approve="reject" data-ew-approval-role="${escape(role)}">${escape(text('reject', '驳回'))}</button></span>`).join('')
      : '';
    const progress = state.progress && state.progress.message ? `<div class="ew-inline-progress" role="status" aria-live="polite">${escape(state.progress.message)}</div>` : '';
    return `<div class="ew-page ew-applications-page">${progress}<div class="ew-page-intro"><div><span class="ew-kicker">${escape(text('nav_applications', '报销申请'))}</span><h2>${escape(text('applications_heading', '申请生命周期'))}</h2></div><div class="ew-inline-actions"><select id="ew-application-type"><option value="daily_expense">${escape(text('daily_expense', '日常费用报销'))}</option><option value="travel_expense">${escape(text('travel_expense', '差旅费报销'))}</option><option value="rental_expense">${escape(text('rental_expense', '房租费报销'))}</option><option value="communication_expense">${escape(text('communication_expense', '通讯费报销'))}</option></select><button type="button" class="btn btn-primary" data-ew-create${actionState(state, 'create')}>${escape(text('create', '新建'))}</button></div></div>
