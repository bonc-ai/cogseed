...
if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
  window.addEventListener('i18n-change', () => {
    ...
    if (!cid) return;
    const list = document.getElementById('chat-queue-list');
    const editingRow = list?.querySelector('.chat-queue-item.editing');
    const editing = editingRow ? {
      qid: editingRow.dataset.qid || '',
      value: editingRow.querySelector('.chat-queue-edit')?.value || '',
      selectionStart: editingRow.querySelector('.chat-queue-edit')?.selectionStart ?? 0,
      selectionEnd: editingRow.querySelector('.chat-queue-edit')?.selectionEnd ?? 0,
      focused: document.activeElement === editingRow.querySelector('.chat-queue-edit'),
    } : null;
    renderMessageQueue(cid);
    if (!editing?.qid) return;
    const nextRow = document.querySelector(`#chat-queue-list .chat-queue-item[data-qid="${CSS.escape(editing.qid)}"]`);
    if (!nextRow) return;
    _startQueueItemEdit(cid, nextRow);
    const textarea = nextRow.querySelector('.chat-queue-edit');
    if (!textarea) return;
    textarea.value = editing.value;
    textarea.setSelectionRange(editing.selectionStart, editing.selectionEnd);
    if (!editing.focused) textarea.blur();
  });
}
