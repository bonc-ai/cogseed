...
function _showBashPermissionModeDialog({ title, message, currentMode, getTitle, getMessage }) {
  let modes = _bashPermissionModeOptions();
  const safeCurrentMode = _bashIsMode(currentMode) ? currentMode : _BASH_PERMISSION_DEFAULT_MODE;
  const localizedTitle = () => typeof getTitle === 'function' ? getTitle() : title;
  const localizedMessage = () => typeof getMessage === 'function' ? getMessage() : message;
  ...
  return new Promise((resolve) => {
    ...
    const updateModeUi = () => {
      ...
      modeOptions.forEach((btn) => {
        ...
        const localized = modes.find((entry) => entry.mode === btn.dataset.mode);
        const label = btn.querySelector('.bash-permission-mode-label');
        const desc = btn.querySelector('.bash-permission-mode-desc');
        if (label && localized) label.textContent = localized.label;
        if (desc && localized) desc.textContent = localized.desc;
      });
    };
    const closeMenu = () => {
...
async function _showBashPermissionDialog(info) {
  ...
  const getTitle = () => t(isAction ? 'bash.permission.action_title' : 'bash.permission.title');
  const getMessage = () => {
