(function touchpointSettingsModule() {
  ...
  function renderDelivery(view) {
    ...
    const briefingStatus = view.briefingConfigured && view.briefingSchedule
      ...
      : '';
    const configuredActions = view.canConfigureDelivery
      ? `<div class="touchpoint-inline-actions">
          ${uiButton({ label: tr('touchpoint_settings.delivery.preview', '预览今日简报'), className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'briefing.preview' } })}
          ${uiButton({ label: tr('touchpoint_settings.delivery.test', '发送测试消息'), role: 'primary', attrs: { 'data-touchpoint-action': 'briefing.test' } })}
          ${uiButton({ label: tr('touchpoint_settings.approval_cards.manage', '审批卡片管理'), icon: 'settings', className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'approval_cards.manage' } })}
        </div>
        <div class="touchpoint-briefing-row">
          <label class="touchpoint-briefing-time">${escape(tr('touchpoint_settings.delivery.time_label', '每日投递时间'))}<input type="time" value="${escape(briefingTime)}" data-touchpoint-briefing-time></label>
          ${uiButton({ label: scheduleLabel, role: 'primary', attrs: { 'data-touchpoint-action': 'briefing.schedule' } })}
          ${view.briefingConfigured ? uiButton({ label: tr('touchpoint_settings.delivery.unschedule', '取消每日简报'), role: 'danger', className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'briefing.unschedule' } }) : ''}
        </div>
        ${briefingStatus ? `<div class="touchpoint-briefing-status">${escape(briefingStatus)}</div>` : ''}`
      : `<div class="touchpoint-locked-note">${iconMarkup('lock', 'touchpoint-lock-icon')}<span>${escape(tr('touchpoint_settings.delivery.locked', '完成连接、授权和资源同步后即可配置。'))}</span></div>`;
    return `<section class="touchpoint-delivery">
      <div class="touchpoint-section-heading"><div><h3>${escape(tr('touchpoint_settings.delivery.title', '离开电脑后，Mate 如何联系你'))}</h3><p>${escape(tr('touchpoint_settings.delivery.subtitle', '移动触点只承担简报、重要提醒、审批和结果回报。'))}</p></div></div>
      <div class="touchpoint-delivery-grid">
        <article><span>${iconMarkup('sun', 'touchpoint-delivery-icon')}</span><div><strong>${escape(tr('touchpoint_settings.delivery.briefing', '今日简报'))}</strong><p>${escape(tr('touchpoint_settings.delivery.briefing_detail', '每天汇总日程、截止风险和建议。'))}</p></div></article>
        <article><span>${iconMarkup('bell', 'touchpoint-delivery-icon')}</span><div><strong>${escape(tr('touchpoint_settings.delivery.reminders', '重要提醒'))}</strong><p>${escape(tr('touchpoint_settings.delivery.reminders_detail', '只在冲突、截止或状态变化时打扰。'))}</p></div></article>
        <article><span>${iconMarkup('check-circle', 'touchpoint-delivery-icon')}</span><div><strong>${escape(tr('touchpoint_settings.delivery.approvals', '确认与审批'))}</strong><p>${escape(tr('touchpoint_settings.delivery.approvals_detail', '事实确认和 Agent 审批同步回桌面端。'))}</p></div></article>
      </div>
      ${configuredActions}
      ${state.preview ? `<div class="touchpoint-preview"><pre>${escape(state.preview.text || '')}</pre></div>` : ''}
    </section>`;
  }
  ...
  function renderTouchpointConfig() {
    ...
    const sceneTabs = scenes.map((scene) => uiButton({
      label: tr(`touchpoint_settings.template.scene.${scene}`, scene),
      role: 'ghost',
      className: `touchpoint-scene-tab${scene === state.touchpointScene ? ' is-active' : ''}`,
      attrs: {
        role: 'tab',
        'aria-selected': scene === state.touchpointScene ? 'true' : 'false',
        'data-touchpoint-scene': scene,
      },
    })).join('');
    return `<div class="touchpoint-approval-layout">
      <main class="touchpoint-template-editor">
        <div class="touchpoint-scene-tabs" role="tablist" aria-label="${escape(tr('touchpoint_settings.template.scene', '场景'))}">${sceneTabs}</div>
        <div class="touchpoint-template-fields">
          <label for="touchpoint-template-title"><span>${escape(tr('touchpoint_settings.template.title_field', '标题'))}</span>${uiInput({ id: 'touchpoint-template-title', value: draft.title, placeholder: tr('touchpoint_settings.template.title_placeholder', '例如：{{actor}} 请求你确认任务'), attrs: { 'data-touchpoint-config-title': '' } })}</label>
          <label for="touchpoint-template-body"><span>${escape(tr('touchpoint_settings.template.body_field', '正文'))}</span>${uiTextarea({ id: 'touchpoint-template-body', value: draft.body, placeholder: tr('touchpoint_settings.template.body_placeholder', '说明需要审批的内容，以及批准后的影响。'), attrs: { 'data-touchpoint-config-body': '' } })}</label>
          <div class="touchpoint-template-button-fields">
            <label for="touchpoint-template-approve"><span>${escape(tr('touchpoint_settings.template.approve_label', '批准按钮'))}</span>${uiInput({ id: 'touchpoint-template-approve', value: draft.approve, attrs: { 'data-touchpoint-config-button': 'approve' } })}</label>
            <label for="touchpoint-template-reject"><span>${escape(tr('touchpoint_settings.template.reject_label', '拒绝按钮'))}</span>${uiInput({ id: 'touchpoint-template-reject', value: draft.reject, attrs: { 'data-touchpoint-config-button': 'reject' } })}</label>
          </div>
        </div>
        <details class="touchpoint-template-variables"><summary>${escape(tr('touchpoint_settings.template.variables_summary', '插入动态信息'))}<small>${escape(tr('touchpoint_settings.template.variables_detail', '卡片发送时会自动替换'))}</small></summary><div class="touchpoint-template-variable-list"><code>{{actor}}</code><code>{{summary}}</code><code>{{task_title}}</code><code>{{impact}}</code></div></details>
        <details class="touchpoint-template-advanced"><summary>${iconMarkup('settings', 'touchpoint-advanced-icon')}<span>${escape(tr('touchpoint_settings.template.advanced', '高级设置'))}<small>${escape(tr('touchpoint_settings.template.advanced_detail', '指定由哪个飞书机器人发送'))}</small></span></summary><div class="touchpoint-template-routing"><label>${escape(tr('touchpoint_settings.template.scene_override', '当前场景单独指定'))}<select data-touchpoint-config-route>${option((config.routes || {})[state.touchpointScene], tr('touchpoint_settings.template.follow_default', '跟随默认发送机器人'))}</select></label><p>${escape(tr('touchpoint_settings.template.routing_help', '通常无需修改。只有配置了多个机器人时，才需要为当前场景单独指定。'))}</p></div></details>
      </main>
      ${renderTouchpointTemplatePreview(draft)}
    </div>`;
  }

  function renderApprovalCardManager() {
    return `<div class="touchpoint-approval-manager"><header class="touchpoint-manager-header">${uiIconButton({ label: tr('touchpoint_settings.approval_cards.back', '返回触达设置'), icon: 'chevron-left', className: 'touchpoint-icon-button', attrs: { 'data-touchpoint-action': 'approval_cards.back' } })}<div><h1>${escape(tr('touchpoint_settings.approval_cards.title', '审批卡片管理'))}</h1><p>${escape(tr('touchpoint_settings.approval_cards.subtitle', '选择使用场景，修改卡片内容，然后保存或发送测试卡片。'))}</p></div><div class="touchpoint-manager-actions">${uiButton({ label: tr('touchpoint_settings.approval_cards.test', '发送测试卡片'), icon: 'send', disabled: state.busy, className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'touchpoint.test' } })}${uiButton({ label: tr('touchpoint_settings.template.save', '保存模板'), role: 'primary', disabled: state.busy, attrs: { 'data-touchpoint-action': 'touchpoint.config.save' } })}</div></header>${state.notice ? `<div class="messaging-notice is-${escape(state.notice.kind)}">${escape(noticeText(state.notice))}</div>` : ''}${renderTouchpointConfig()}<section class="touchpoint-template-help"><div>${iconMarkup('check-circle', 'touchpoint-governance-icon')}</div><div><strong>${escape(tr('touchpoint_settings.approval_cards.help_title', '只需要关注卡片文案'))}</strong><p>${escape(tr('touchpoint_settings.approval_cards.help_detail', '审批结果、按钮动作和安全校验由系统处理；机器人选择仅在多机器人场景下需要调整。'))}</p></div></section></div>`;
  }
  ...
  function render() {
    ...
    const view = model();
    const dashboard = state.dashboard;
    host.innerHTML = `<div class="touchpoint-settings"><header class="touchpoint-hero"><div><h1>${escape(tr('touchpoint_settings.title', '飞书移动触点'))}</h1><p>${escape(tr('touchpoint_settings.subtitle', '桌面端负责完整工作，飞书负责你离开电脑后的提醒、确认和结果回报。'))}</p></div><div class="touchpoint-hero-actions"><span class="touchpoint-status is-${view.status}"><span></span>${escape(statusText(view.status))}</span>${uiButton({ label: tr('touchpoint_settings.connection.manage', '连接管理'), className: 'touchpoint-secondary touchpoint-manage-button', attrs: { 'data-touchpoint-action': 'connection.manage' } })}${uiIconButton({ label: tr('touchpoint_settings.refresh', '刷新'), icon: 'refresh', className: 'touchpoint-icon-button', attrs: { 'data-touchpoint-action': 'refresh' } })}</div></header>${renderSteps(view)}<section class="touchpoint-next"><div><span>${escape(tr('touchpoint_settings.next_label', '当前下一步'))}</span><h2>${escape(primaryText(view.primaryAction))}</h2><p>${escape(tr(`touchpoint_settings.next_detail.${view.primaryAction}`, ''))}</p></div>${uiButton({ label: primaryText(view.primaryAction), role: 'primary', disabled: state.busy, attrs: { 'data-touchpoint-action': view.primaryAction } })}</section>${view.syncMessage ? `<div class="messaging-notice is-error">${escape(view.syncMessage)}</div>` : ''}<div class="touchpoint-overview">${renderConnectionCard(view)}${renderAccessCard(view)}</div>${renderSetupGuideCard()}${renderResourcePicker()}${renderDelivery(view)}<section class="touchpoint-governance"><div>${iconMarkup('shield', 'touchpoint-governance-icon')}</div><div><strong>${escape(tr('touchpoint_settings.governance.title', '你始终掌握控制权'))}</strong><p>${escape(tr('touchpoint_settings.governance.detail', '默认只读；授权范围可查看、可撤销；所有外部写入和发送都需要明确确认。'))}</p></div></section>${state.notice ? `<div class="messaging-notice is-${escape(state.notice.kind)}">${escape(noticeText(state.notice))}</div>` : ''}</div>`;
    hydrate(host);
  }
  ...
  function renderSetupGuideCard() {
    ...
    return `<section class="touchpoint-setup-guide"><div class="touchpoint-section-heading"><div><h3>${escape(tr('touchpoint_settings.setup_guide.title', '还差最后一步：配置回调地址'))}</h3><p>${escape(tr('touchpoint_settings.setup_guide.desc', '飞书要求回调地址与应用后台配置完全一致，不配置则授权时会报错（一次性设置，约 30 秒）：'))}</p></div></div><div class="touchpoint-setup-guide-url"><code>${escape(guide.redirectUri || '')}</code>${uiButton({ label: tr('touchpoint_settings.setup_guide.copy', '复制地址'), className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'setup_guide.copy' } })}</div><p class="touchpoint-setup-guide-steps">${escape(tr('touchpoint_settings.setup_guide.steps', '在打开的「安全设置 → 重定向 URL」中粘贴上面的地址 → 点「添加」→ 点「发布」即可。'))}</p><div class="touchpoint-inline-actions">${uiButton({ label: tr('touchpoint_settings.setup_guide.open_console', '打开开发者后台'), className: 'touchpoint-secondary', attrs: { 'data-touchpoint-action': 'setup_guide.open' } })}${uiButton({ label: tr('touchpoint_settings.setup_guide.done', '我已配置好'), role: 'primary', attrs: { 'data-touchpoint-action': 'setup_guide.done' } })}</div></section>`;
  }

  async function runAction(action) {
    if (state.busy) return;
    if (action === 'refresh') return refresh();
    if (action === 'overview.open') { showOverview(); return; }
  ...
  window.initTouchpointSettings = async function initTouchpointSettings() {
    bind();
    // IM opens on the real multi-channel manager. The touchpoint overview
    // remains available from the manager header for authorization and delivery setup.
