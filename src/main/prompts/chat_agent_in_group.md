...
- Include exactly one internal task result marker in every final reply: `<agent-result status="success" />` when you completed the expected outcome; `<agent-result status="waiting_input" />` when the task remains unfinished because you need user input or a required dependency; `<agent-result status="failure" />` when you attempted the task but did not complete the expected outcome or satisfy the delivery standards. Do not use these markers for runtime/tool exceptions; the system records those independently. If your reply contains `<agent-input-form>`, put the `waiting_input` marker before the form block.
