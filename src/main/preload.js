...
const PUSH_EVENT_PREFIXES = ['marketplace:', 'conversations:', 'connectors:', 'client-config:', 'delete_file.', 'bridge:', 'bash:', 'action-approval:', 'interactive-cli:', 'messaging:', 'personal-context:', 'hub-account:', 'local-agents:', 'updates:', 'workspace-import:'];
...
const cogseedApi = {
  ...
  // `language` is reserved for Commander/Agent response language. UI code
  // must use the explicit UI variants below so the two settings cannot drift.
  getLanguage: () => invoke('config.getLanguage'),
  setLanguage: (language) => invoke('config.setLanguage', { language }),
  getUiLanguage: () => invoke('config.getUiLanguage'),
  setUiLanguage: (uiLanguage) => invoke('config.setUiLanguage', { uiLanguage }),
