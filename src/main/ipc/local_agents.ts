...
import * as launchConfirm from '../features/local_agents/launch_confirm.js';
...
export const invokeHandlers = {
  ...
  /** Renderer answer to a `local-agents:launch-confirm` push event (external
   *  CLI agent first-launch gate). Unknown / already-timed-out request ids
   *  return handled:false (stale dialog). Verdict semantics live in
   *  features/local_agents/launch_confirm.ts. */
  'local-agents.launch_confirm_response': async (
    payload: { request_id?: unknown; allow?: unknown; always?: unknown },
  ) => {
    if (typeof payload?.request_id !== 'string' || !payload.request_id) throw new Error('invalid request_id');
    if (typeof payload?.allow !== 'boolean') throw new Error('invalid allow flag');
    const handled = launchConfirm.respond(payload.request_id, payload.allow, payload?.always === true);
    return { handled };
  },
