...
export interface MateExecutionRecord {
  ...
  boundary: 'real';
  permissionMode: 'mate-agent-runtime';
  eventCount?: number;
...
function validateRecord(userId: string, executionId: string, value: unknown): MateExecutionRecord {
  ...
  if (row.kind !== 'mate-agent' || row.boundary !== 'real' || row.permissionMode !== 'mate-agent-runtime') throw new Error('malformed Mate execution record');
