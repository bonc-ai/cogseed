...
function mateScope(scope: CollaborationScope): { userId: string; coordinationId: string } {
  if (scope.domain !== 'mate') throw new Error('Mate collaboration store requires mate domain');
...
export function createMateCollaborationStore(): CollaborationStore {
  return {
    ...
    writeRun(scope, run) { const s = mateScope(scope); if (run.cid !== s.coordinationId) throw new Error('Mate collaboration run scope mismatch'); return writeJson(mateCoordinationControlRunFile(s.userId, s.coordinationId), run); },
    ...
    writeContext(scope, context) { const s = mateScope(scope); if (context.cid !== s.coordinationId) throw new Error('Mate collaboration context scope mismatch'); return writeJson(mateCoordinationControlContextFile(s.userId, s.coordinationId), context); },
    async appendEvent(scope, event) { const s = mateScope(scope); if (event.cid !== s.coordinationId) throw new Error('Mate collaboration event scope mismatch'); await appendJsonlAtomic(mateCoordinationControlEventsFile(s.userId, s.coordinationId), event); },
