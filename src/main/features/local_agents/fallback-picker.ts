 ...
 *
 *   1. Prefer an explicit `prefer` type when it passes the usable filter
 *      (extraction historically preferred Claude Code).
 *   2. Otherwise prefer a SIGNED-IN CLI (official account / API-key config).
 *   3. Otherwise fall back to the first available CLI — the credential check
 *      is file-based and can miss keychain-stored sessions, so an available
 *      CLI is still a valid fallback backend (it will surface its own login
...
export async function pickBestCliForFallback(
  ...
  return signedIn || candidates[0] || null;
}
