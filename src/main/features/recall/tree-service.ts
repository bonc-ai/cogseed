...
import { validatePromotionByAssetType } from './formal-assets';
 ...
 * 一条候选现在是不是「芽」。
 *
 * 判据只有一条**已有**的唯一来源：`getRecallCandidateCapabilities().canPromote`
 * ——「用户现在能不能把它确认成正式资产」，加上晋升闸门 `validatePromotionByAssetType`
 * ——「够不够格成为该类型的正式资产」。两者都过才画芽。任何一条不过，芽就不该
 * 出现在树上：树上出现一个点不去，或者点了也进不了正式资产的芽，就是假闭环。
 *
 ...
 * 树会比真实可做的事更保守，同样是撒谎。
 */
function isBudCandidate(candidate: RecallCandidateRecord, presentAssetIds: ReadonlySet<string>): boolean {
  ...
  if (!getRecallCandidateCapabilities(candidate).canPromote) return false;
  return validatePromotionByAssetType({
...
export async function rebuildCognitionTree(userId: string): Promise<CognitionTreeRecord> {
  ...
  const candidateNodes: CognitionTreeCandidateNode[] = [...allCandidates]
    .filter((candidate) => isBudCandidate(candidate, assetIds))
    ...
    .map((candidate) => ({
      ...
      displayState: getRecallCandidateCapabilities(candidate).displayState,
      risk: candidate.risk,
    }));
