...
function asAsset(value: RecallJsonRecord): RecallAbilityAssetRecord {
  ...
  return { ...value, evidenceRefs, ...(learningSignal ? { learningSignal } : {}), ...(ontologyRefs ? { ontologyRefs } : {}), ...relationContract, ...(scopePolicy ? { scopePolicy } : {}) } as RecallAbilityAssetRecord;
}
