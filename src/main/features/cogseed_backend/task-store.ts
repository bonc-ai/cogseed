...
function validateTask(userId: string, value: unknown, expectedTaskId?: string): MateTaskRecord {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('malformed Mate task');
  const row = value as Record<string, unknown>;
  if (row.schemaVersion !== MATE_AGENT_BACKEND_SCHEMA_VERSION) throw new Error('unsupported Mate task schema');
  if (row.ownerId !== userId) throw new Error('Mate task owner mismatch');
  if (typeof row.taskId !== 'string') throw new Error('malformed Mate task');
  assertMateTaskId(row.taskId);
  if (expectedTaskId && row.taskId !== expectedTaskId) throw new Error('Mate task id mismatch');
  if (typeof row.sessionId !== 'string' || typeof row.runtimeSessionId !== 'string' || typeof row.requestId !== 'string') {
    throw new Error('malformed Mate task');
  }
  if (!row.runtimeSessionId.startsWith('mruntime-')) throw new Error('malformed Mate task');
  if (row.executionId !== undefined && (typeof row.executionId !== 'string' || !row.executionId.startsWith('mate-exec-'))) throw new Error('malformed Mate task');
  if (row.runtimeRunId !== undefined && (typeof row.runtimeRunId !== 'string' || !row.runtimeRunId.startsWith('run_'))) throw new Error('malformed Mate task');
  if (row.runtimeWorkerId !== undefined && (typeof row.runtimeWorkerId !== 'string' || !row.runtimeWorkerId.startsWith('mate-worker-'))) throw new Error('malformed Mate task');
  if (row.coordinationId !== undefined && (typeof row.coordinationId !== 'string' || !row.coordinationId.startsWith('mate-coord-'))) throw new Error('malformed Mate task');
  if (row.parentTaskId !== undefined && (typeof row.parentTaskId !== 'string' || !row.parentTaskId.startsWith('mate-task-'))) throw new Error('malformed Mate task');
  if (row.coordinationDepth !== undefined && (!Number.isInteger(row.coordinationDepth) || Number(row.coordinationDepth) < 1)) throw new Error('malformed Mate task');
...
function validateClaim(userId: string, requestId: string, value: unknown): MateRequestClaim {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('malformed Mate request claim');
  const row = value as Record<string, unknown>;
  if (row.schemaVersion !== MATE_AGENT_BACKEND_SCHEMA_VERSION || row.ownerId !== userId || row.requestId !== requestId) {
    throw new Error('malformed Mate request claim');
  }
  if (typeof row.taskId !== 'string' || typeof row.createdAt !== 'string') throw new Error('malformed Mate request claim');
  assertMateTaskId(row.taskId);
...
export async function readMateTask(userId: string, taskId: string): Promise<MateTaskRecord | null> {
  ...
  try {
    ...
    if (error instanceof SyntaxError) throw new Error('malformed Mate task');
    throw error;
  }
}
...
export async function readMateTaskByRequestId(userId: string, requestId: string): Promise<MateTaskRecord | null> {
  ...
  try {
    ...
    if (isEnoent(error)) return null;
    if (error instanceof SyntaxError) throw new Error('malformed Mate request claim');
    throw error;
  }
}
...
export async function listMateTasks(userId: string): Promise<MateTaskRecord[]> {
  ...
  for (const entry of entries) {
    ...
    tasks.push(await readMateTask(userId, taskId).then((task) => { if (!task) throw new Error('Mate task disappeared during recovery'); return task; }));
  }
...
export async function createMateTask(userId: string, input: CreateMateTaskInput): Promise<CreateMateTaskResult> {
  ...
  if (!task) throw new Error('Mate task is required');
  ...
  return fileEditLock(claimFile).runExclusive(async () => {
    try {
      ...
      if (!existing) throw new Error('Mate request claim references a missing task');
    ...
    const taskRecord: MateTaskRecord = {
      ...
      ...(input.coordinationDepth !== undefined ? { coordinationDepth: (() => { const depth = Number(input.coordinationDepth); if (!Number.isInteger(depth) || depth < 1) throw new Error('invalid Mate coordination depth'); return depth; })() } : {}),
      createdAt,
      updatedAt: createdAt,
    };
