...
export const TOOL_CATALOG = Object.freeze<readonly RuntimeToolCatalogEntry[]>([
  ...
  },
  {
    ...
    summary: 'List tools exposed by the user-enabled Mate connectors.',
    kind: 'skill',
    parameters: { type: 'object', properties: { connector_id: { type: 'string' } }, additionalProperties: false },
  },
  {
    name: 'call_connector_tool',
    summary: 'Call one user-enabled Mate connector tool by connector id and tool name.',
    kind: 'skill',
  ...
  {
    name: 'search_mate_kb',
    summary: 'Search the user-owned Mate knowledge base.',
    kind: 'file',
  ...
  {
    name: 'read_mate_kb',
    summary: 'Read one user-owned Mate knowledge base source.',
    kind: 'file',
  ...
  { name: 'browser_open', summary: 'Open a public HTTP or HTTPS page in the isolated Mate browser.', kind: 'host', parameters: { type: 'object', properties: { url: { type: 'string' } }, required: ['url'], additionalProperties: false } },
  ...
  { name: 'mate_delegate', summary: 'Delegate an explicit bounded subtask to one child Mate Agent task.', kind: 'host', parameters: { type: 'object', properties: { task: { type: 'string' }, role: { type: 'string' }, context: { type: 'array' } }, required: ['task'], additionalProperties: false } },
  { name: 'mate_tasks', summary: 'Read status summaries for child tasks in the current Mate coordination.', kind: 'host', parameters: { type: 'object', properties: { task_ids: { type: 'array', items: { type: 'string' } } }, required: ['task_ids'], additionalProperties: false } },
  { name: 'mate_cancel', summary: 'Cancel one child task in the current Mate coordination.', kind: 'host', parameters: { type: 'object', properties: { task_id: { type: 'string' } }, required: ['task_id'], additionalProperties: false } },
  { name: 'mate_retry_step', summary: 'Retry and dispatch one failed or blocked Mate workflow step.', kind: 'host', parameters: { type: 'object', properties: { step_id: { type: 'string' } }, required: ['step_id'], additionalProperties: false } },
