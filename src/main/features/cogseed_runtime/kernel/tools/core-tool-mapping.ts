/**
 * Explicit Core Agent → Mate Runtime tool contract.
 *
 * This is intentionally separate from both catalogs: Core owns the historical
 * public tool names, while Mate owns the executable Runtime catalog. The matrix
 * makes every compatibility decision reviewable and gives anti-drift tests one
 * place to prove that a Runtime tool is either mapped or explicitly Mate-native.
 */
...
export interface CoreToMateToolMapping {
  ...
  /** Executable Mate Runtime names, empty for deferred tools. */
...
export const CORE_TO_MATE_TOOL_MAPPINGS: readonly CoreToMateToolMapping[] = Object.freeze([
  ...
  deferred('publish_outputs', 'Deferred until Mate output registration and renderer-safe publication semantics are available.'),
  ...
  replacement('create_docx', ['office_create'], 'Mate uses one validated Office create host capability; the adapter selects the document format from the target path.'),
  replacement('create_xlsx', ['office_create'], 'Mate uses one validated Office create host capability; the adapter selects the workbook format from the target path.'),
  replacement('create_pptx', ['office_create'], 'Mate uses one validated Office create host capability; the adapter selects the presentation format from the target path.'),
  parity('office_read', 'Same Office read capability through the Main-side adapter and reverse host-tool protocol.'),
  replacement('edit_office', ['office_edit'], 'Mate uses one validated Office edit host capability with the same path and batch safety boundary.'),
  parity('office_render', 'Same Office preview capability through the Main-side adapter and reverse host-tool protocol.'),
  deferred('kb_list', 'Deferred until the Mate KB adapter exposes a bounded source/status listing contract.'),
  replacement('kb_search', ['search_mate_kb'], 'Mate-owned KB search preserves user scoping and uses the Runtime KB manager boundary.'),
  replacement('kb_read', ['read_mate_kb'], 'Mate-owned KB source reads preserve user scoping and use the Runtime KB manager boundary.'),
  ...
  deferred('video_studio', 'Deferred because VideoStudio is a native Main-side capability with a separate owner-scoped lifecycle.'),
  ...
  replacement('manage_execution_plan', ['mate_workflow', 'mate_retry_step', 'mate_skip_step', 'mate_resume_workflow'], 'Mate workflow inspection and auditable step controls replace the Core plan tool without importing the legacy state machine.'),
...
/** Runtime tools that are intentionally Mate-native additions, not Core aliases. */
