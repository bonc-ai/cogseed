...
async function _buildCliPrompt(
  ...
  const importedSummary = await _extractImportedCliContext(uid, cid, projectId);
  if (importedSummary) {
    sliceLines.unshift(
...
async function _extractImportedCliContext(uid: string, cid: string, projectId?: string): Promise<string> {
  const re = /以下是用户从[^\n]*提炼简报[^\n]*\n*\s*([\s\S]+)/;
  try {
    const chats = await import("../chats");
