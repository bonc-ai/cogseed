...
async function hostRouteTaskTurn(
  ...
  try {
    ...
    // runner over the committed projection knowledge). Never blocks the turn
    // — ok:false just means execution proceeds without a forecast record.
    const { autoForecastForRequirement } = await import('../kstar/auto-forecast');
    await autoForecastForRequirement(uid, cid, created.requirementId);
...
async function ensureKstarTaskForDispatch(
  ...
  try {
    ...
    // World-model prediction: forecast is generated by the host, not by the
    // Commander (dedicated runner; never blocks the dispatch).
    const { autoForecastForRequirement } = await import('../kstar/auto-forecast');
    await autoForecastForRequirement(uid, cid, result.requirementId);
    return {
      created: true,
