...
function _firstNonBlankText(...candidates: Array<string | null | undefined>): string {
...
async function runActorTurnBody(
  ...
  // Keep the switched-agent digest separate from the LLM payload so the same
  // handoff context can be sent to both in-process and P3394 gateway agents.
  let switchedContextDigest = "";
  ...
  // A directly targeted Agent may have no visibility of the conversation that
  ...
  if (!cliAgent) {
    if (isCommander) {
      try {
        ...
        // 回执 = 「本回合真的注入了这几条」这个事实，注入发生了就该落盘；
        // reuseTurnIds = 本次运行的簿记，只有确实开着 taskRun 时才登记。
        // 两者此前共用一道 `state.taskRun` 门，导致实机上自动投影注入 26 次、
        // 回执只落 4 张：终态永远找不到凭证，连 M-6 恢复也只能扫到不存在的
        // 回执。出生继承那条路径（本函数下方）早就是这个形状，这里对齐它。
        if (recallCitations.length) {
      ...
      try {
        ...
        if (dispatched.assets.length) {
          ...
          // 与投影注入同一口径：回执按「确实派发了资产」落，reuseTurnIds 才看
          // 有没有 taskRun——否则无任务运行时派发的资产不留任何加载凭证。
          {
            try {
              const { prepareReceipt } = await import('../p3394/context-reuse-receipt');
              const prepared = await prepareReceipt(
  ...
  if (cliAgent) {
    ...
    try {
      ...
      // 外部智能体首次唤起确认：CLI 智能体（含 P3394 网关）第一次把消息交给
      // 本机外部 CLI 程序前必须得到用户明确同意——首装无 API Key 时 fallback
      // 自动创建的智能体、显式 @ 唤起的智能体、以及 commander 首次派发给外部
      // 智能体的任务都走这里。确认只按 (agent_id, cli) 询问一次（「始终允许」
      ...
      const cliRuntime =
        cliAgent.runtime?.kind === "cli" || cliAgent.runtime?.kind === "p3394-gateway"
          ...
          : "";
      if (cliRuntime) {
        const launchConfirmed = await (
          await import("../local_agents/launch_confirm")
        ).requestLaunchConfirm({
          uid,
          cid,
          agentId: cliAgent.agent_id,
          agentName: cliAgent.name || cliAgent.agent_id,
          cli: cliRuntime,
        });
        if (!launchConfirmed) {
          const reply = `<span style="color:var(--danger)">${escapeHtmlForBubble(t("cli_launch_confirm.denied", { name: cliAgent.name || cliAgent.agent_id }))}</span>`;
          log.warn("external agent launch denied by user", {
            cid,
  ...
  // inputs to a later stage. Explicitly published outputs are different:
  // review artifacts must remain visible before the user approves the next stage.
