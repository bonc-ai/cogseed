...
export interface GroupMessage {
  ...
  /** Structured collaboration summary payload for `system_kind:
   * 'collab_summary'` messages. Drives the renderer summary card; `text`
   * stays a readable fallback for plain-text consumers. */
  collab_summary?: CollabSummaryRecord;
...
/** Multi-agent collaboration end-of-run summary, generated by the host when a
 * workflow run reaches an effective terminal state (all steps done / failed /
 * cancelled). Persisted as a `system_kind: 'collab_summary'` group message so
 * the renderer renders a summary card and agent workers see the conclusion on
 * slice replay. Numbers and bounded text only. */
export interface CollabSummaryRecord {
  version: 1;
  run_id: string;
  cid: string;
  conclusion: "all_steps_done" | "cancelled" | "failed";
  objective: string;
  started_at: string;
  ended_at: string;
  step_totals: {
  ...
  anomalies: Array<{
    ts: string;
    kind: "failure" | "retry" | "cancel" | "degraded";
    actor_id?: string;
    step_id?: string;
    detail: string;
  }>;
