 ...
 *
 * WHY THIS EXISTS
 * skill-sentry ships in this repository under Apache-2.0, but a distribution
 * may still omit it: a slim open-source build drops the engine to keep the
 * payload small, or an operator keeps a separately-maintained scanner variant
 * for their deployment. Step 1 made absence expressible (`scanner_absent`);
 * this step lets such a build still find a scanner when the operator has
 * installed one separately as a skill package.
 *
 * WHAT IT IS NOT
 ...
 * skill keeps the resolution uniform for every deployment shape: whether the
 * engine ships in the build or is installed separately, the app sees a skill id
 * and a JSON contract.
 *
...
function candidateRoots(uid: string | null): string[] {
  ...
  return [
    // Explicit operator override wins: a deployment points this at wherever
    // it keeps its separately-installed scanner engine.
    ...(override ? [override] : []),
 ...
 *
 * Prefers one shipped alongside the engine, so a separately-maintained scanner
 * package can evolve its own decision script. Otherwise falls back to the
 * repository's own copy — open source, containing no rules, it reads the
 * engine's report and
