...
export async function importClaudeSkill(dirName: string): Promise<ImportSkillResult> {
  ...
  // Skill skeleton conversion: auto-generate the missing standard artifacts
  // (same as the skill-library import path — onboarding-imported skills must
  // carry the full contract too).
  try {
    ensureSkillSkeleton(path.join(userSkillsDir(getActiveUserId()), skillId), name);
  } catch (err) {
    log.warn('session-import skill skeleton generation failed', { skillId, error: String(err) });
  }
