...
async function runCliExtractionPass(userId: string, systemPrompt: string, content: string): Promise<string | null> {
  try {
    ...
    // 优先采用用户在设置里保存的「降级执行 Agent」（cli-fallback 偏好），
    // 否则按「已登录 CLI → 任意可用」挑选。每个跑失败的 CLI 会被排除并
    // 换下一个已登录 CLI 继续尝试，直到全部试完才降级为「未提炼」——不再
    // 硬编码 Claude、也不再让一个没登录的 Claude 挡住已登录的 WorkBuddy。
    const prefer = getCliFallback(userId) || undefined;
    const tried = new Set<string>();

    let chosen;
    while ((chosen = await pickBestCliForFallback({ prefer, exclude: tried }))) {
      if (tried.has(chosen.type)) break; // 防御：pick 不应返回已排除的 CLI
      ...
      try {
        ...
        // 失败/取消：继续尝试下一个可用 CLI（当前 CLI 已加入 exclude）。
      } finally {
