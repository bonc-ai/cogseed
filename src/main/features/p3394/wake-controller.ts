...
export interface DecideWakeRequestInput { requestId: string; decision: 'approve' | 'reject'; reason?: string }
...
export async function decideWakeRequest(userId: string, input: DecideWakeRequestInput, deps: DecideWakeRequestDeps = {}): Promise<DecideWakeRequestResult> {
  try {
    ...
    if (deps.validateTarget) { if (!(await deps.validateTarget(userId, existing.agent_id))) return { ok: false, error: 'wake target agent is unavailable' }; }
    else { const target = await getAgent(existing.agent_id); if (!target || !isAgentEnabled(userId, existing.agent_id)) return { ok: false, error: 'wake target agent is unavailable' }; targetInteractive = target.interactive === true; }
