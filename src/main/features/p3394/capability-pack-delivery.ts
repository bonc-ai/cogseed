/** DeliveryCapabilityPack：把用户已确认的认知资产，冻结成一份可交给执行端的最小能力包。
 *
...
export interface DeliveryCapabilityPack {
  packId: string;
...
export function buildCapabilityPack(input: BuildCapabilityPackInput): DeliveryCapabilityPack {
...
export function isPackExpired(pack: DeliveryCapabilityPack, at: string | number = Date.now()): boolean {
 ...
 *  失败即拒绝——不做「尽力而为」的降级读取。 */
export function assertPackIntegrity(pack: DeliveryCapabilityPack): void {
