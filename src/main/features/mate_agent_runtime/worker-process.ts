...
const log = createLogger('mate-runtime:worker-process');
...
function defaultSpawnWorker(): RuntimeWorkerChild {
  ...
  const script = path.join(pcDir, 'bin', 'mate-runtime-worker.cjs');
...
export const defaultRuntimeWorkerService = createRuntimeWorkerService({
  hostToolHandler: async (call, context) => {
    const { mateHostToolRouter } = await import('../mate_agent_backend/host-tool-router');
    return mateHostToolRouter.handle(call, context);
  },
  onRunSettled: async (request) => {
    const { mateBrowserManager } = await import('../mate_agent_backend/browser-manager');
