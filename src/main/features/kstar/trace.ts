...
export async function readKstarTrace(
  ...
  if (!safeId(userId) || !safeId(input.conversationId) || (input.taskId !== undefined && !safeId(input.taskId))) throw new Error('invalid KSTAR trace reference');
  const state = await readConversationTaskState(userId, input.conversationId);
  const loadedTask = input.taskId
    ? await readKstarTask(userId, input.taskId)
    : state?.currentTaskId ? await readKstarTask(userId, state.currentTaskId) : null;
  const task = loadedTask && loadedTask.conversationId === input.conversationId ? loadedTask : null;
  const tasks = task ? [task] : (await listKstarJsonRecords(userId, 'tasks')).filter((record) => record.conversationId === input.conversationId) as typeof task[];
  ...
  for (const currentTask of tasks) {
    ...
    for (const requirement of requirements) {
      ...
      if (projectionId) {
        const projection = await readRecallJsonRecord(userId, 'context-projections', projectionId).catch(() => undefined);
        nodes.push(node('projection', { status: status((projection as Record<string, unknown> | undefined)?.status), at: String((projection as Record<string, unknown> | undefined)?.decidedAt || (projection as Record<string, unknown> | undefined)?.createdAt || ''), primaryId: projectionId, parentId: requirement.id, source: 'recall' }));
      ...
      for (const episodeId of requirement.episodeIds) {
        ...
        nodes.push(node('extraction', { status: extraction ? status((extraction as Record<string, unknown>).status) : 'not_started', at: String((extraction as Record<string, unknown> | undefined)?.updatedAt || ''), primaryId: extraction ? String(extraction.id) : undefined, parentId: episodeId, source: 'kstar', ...(extraction && typeof extraction.error === 'string' ? { degradedReason: bounded(extraction.error) } : {}) }));
      }
    }
  }
  ...
  const failures = (await listKstarFailures(userId)).filter((failure) => (
  ...
  for (const failure of failures.slice(0, 50)) nodes.push(node('failure', { status: 'failed', at: failure.at, primaryId: failure.id, parentId: failure.episodeId || failure.requirementId || failure.taskId, source: failure.stage, errorCode: failure.errorCode, summary: bounded(failure.errorMessage) }));
  return { conversationId: input.conversationId, ...(task ? { taskId: task.id } : {}), nodes, generatedAt: nowIso() };
}
