 ...
 *
 *   openSession  → getOrCreateCogSeedSession (real session-store record)
 *   deliver      → cogseedRuntimeController.startCogSeedTask (task admission)
 *   stream       → readCogSeedTaskEvents + readCogSeedTask (event/status polling)
 *   resume       → cogseedRuntimeController.resumeCogSeedTask (recoverable tasks)
 *   cancel       → cogseedRuntimeController.cancelCogSeedTask
 *   snapshot     → listCogSeedTasks over the session
 *   closeSession → recordCogSeedTaskRunForRecall for terminal tasks
 *
 ...
 * CogSeed ids (`cogseed-session-*`, `cogseed-task-*`) remain the storage truth.
...
import { getOrCreateCogSeedSession, readCogSeedSession } from '../cogseed_backend/session-store';
import {
  cogseedRuntimeController,
  type CogSeedRuntimeController,
} from '../cogseed_backend/runtime-controller';
import { listCogSeedTasks, readCogSeedTask } from '../cogseed_backend/task-store';
import { readCogSeedTaskEvents } from '../cogseed_backend/event-store';
import { recordCogSeedTaskRunForRecall } from '../cogseed_backend/recall-bridge';
import type { CogSeedTaskEvent } from '../cogseed_backend/types';
...
export interface P3394CogseedRuntimeAdapterDeps {
  ...
  controller?: CogSeedRuntimeController;
  ...
  readEvents?: (userId: string, taskId: string, afterSequence: number, limit: number) => Promise<CogSeedTaskEvent[]>;
}

export interface P3394CogseedAdapterState {
  schemaVersion: 1;
  sessions: Array<{ p3394_session_id: string; cogseed_session_id: string; agent_id?: string }>;
  tasks: Array<{ p3394_task_id: string; cogseed_task_id: string; p3394_session_id: string }>;
}
...
export class P3394CogseedRuntimeAdapter implements P3394RuntimeAdapter {
  ...
  private readonly controller: CogSeedRuntimeController;
  private readonly now: () => string;
  private readonly pollIntervalMs: number;
  private readonly streamTimeoutMs: number;
  private readonly stateFile: string | null;

  /** p3394 session_id → CogSeed cogseed-session-* id. */
  ...
  /** p3394 task_id → CogSeed cogseed-task-* id. */
  ...
  constructor(deps: P3394CogseedRuntimeAdapterDeps = {}) {
    ...
    this.controller = deps.controller ?? cogseedRuntimeController;
    ...
    this.readEvents = deps.readEvents ?? readCogSeedTaskEvents;
  ...
  private loadStateSync(): void {
    try {
      ...
      for (const item of parsed.sessions ?? []) {
        if (typeof item.p3394_session_id !== 'string' || typeof item.cogseed_session_id !== 'string') continue;
        this.sessionMap.set(item.p3394_session_id, item.cogseed_session_id);
        if (typeof item.agent_id === 'string') this.sessionAgentMap.set(item.p3394_session_id, item.agent_id);
      }
      for (const item of parsed.tasks ?? []) {
        if (typeof item.p3394_task_id !== 'string' || typeof item.cogseed_task_id !== 'string') continue;
        this.taskMap.set(item.p3394_task_id, item.cogseed_task_id);
  ...
  private persistState(): void {
    ...
    try {
      const state: P3394CogseedAdapterState = {
        schemaVersion: 1,
        sessions: [...this.sessionMap.entries()].map(([p3394_session_id, cogseed_session_id]) => ({
          p3394_session_id,
          cogseed_session_id,
          ...(this.sessionAgentMap.get(p3394_session_id) ? { agent_id: this.sessionAgentMap.get(p3394_session_id) } : {}),
        })),
        tasks: [...this.taskMap.entries()].map(([p3394_task_id, cogseed_task_id]) => ({
          p3394_task_id,
          cogseed_task_id,
  ...
  private async cogseedSessionFor(sessionId: string, agentId?: string): Promise<string> {
    ...
    if (existing) {
      ...
      if (!this.verifiedSessions.has(existing)) {
        const record = await readCogSeedSession(this.userId(), existing).catch(() => null);
        if (!record) {
          log.warn('P3394 adapter restored cogseed session missing; recreating', {
            p3394_session_id: sessionId,
            cogseed_session_id: existing,
          });
    ...
    const record = await getOrCreateCogSeedSession(this.userId());
  ...
  async openSession(input: { session_id: string; agent_id: string }): Promise<P3394RuntimeSessionBinding> {
    const cogseedSessionId = await this.cogseedSessionFor(input.session_id, input.agent_id);
    return { session_id: input.session_id, native_session_id: cogseedSessionId, agent_id: input.agent_id };
  }
  ...
  async deliver(envelope: P3394Envelope): Promise<{ task_id: string }> {
    ...
    const cogseedSessionId = await this.cogseedSessionFor(envelope.session_id, recipientAgentId);
    const agentId = this.sessionAgentMap.get(envelope.session_id);

    const task = await this.controller.startCogSeedTask(userId, {
      requestId: `req-${genId12()}`,
      task: text.slice(0, 64_000),
      sessionId: cogseedSessionId,
      ...(agentId ? { agentId } : {}),
    });
  ...
  async *stream(taskId: string, afterSequence = 0): AsyncIterable<P3394RuntimeEvent> {
    const cogseedTaskId = this.taskMap.get(taskId);
    if (!cogseedTaskId) throw new Error('p3394_task_not_found');
    const userId = this.userId();
    ...
    for (;;) {
      const events = await this.readEvents(userId, cogseedTaskId, lastSequence, 200);
      for (const event of events) {
        lastSequence = event.sequence;
        if (event.sequence <= startSequence) continue;
        const mapped = mapCogSeedTaskEvent(event);
        if (mapped) {
          sequence = Math.max(sequence, event.sequence);
          yield { ...mapped, sequence, task_id: taskId };
        }
      }
      const task = await readCogSeedTask(userId, cogseedTaskId);
      if (task) {
  ...
  async resume(sessionId: string): Promise<void> {
    const userId = this.userId();
    const cogseedSessionId = this.sessionMap.get(sessionId);
    if (!cogseedSessionId) throw new Error('p3394_session_not_found');
    const recoverable = (await listCogSeedTasks(userId))
      .filter((task) => task.sessionId === cogseedSessionId && task.status === 'recoverable');
    if (recoverable.length === 0) throw new Error('p3394_no_recoverable_task');
    const target = recoverable[0];
    await this.controller.resumeCogSeedTask(userId, target.taskId, {
      requestId: `req-${genId12()}`,
  ...
  async cancel(taskId: string): Promise<void> {
    const cogseedTaskId = this.taskMap.get(taskId);
    if (!cogseedTaskId) throw new Error('p3394_task_not_found');
    await this.controller.cancelCogSeedTask(this.userId(), cogseedTaskId);
  }
  ...
  async snapshot(sessionId: string): Promise<P3394RuntimeSnapshot> {
    ...
    const tasks = (await listCogSeedTasks(userId))
      .filter((task) => task.sessionId === cogseedSessionId)
      .map((task) => ({ task_id: task.taskId, status: task.status }));
    return {
      session_id: sessionId,
      native_session_id: cogseedSessionId,
      at: this.now(),
      state: { tasks },
    };
  }
  ...
  async closeSession(sessionId: string): Promise<void> {
    ...
    const cogseedSessionId = this.sessionMap.get(sessionId);
    if (!cogseedSessionId) throw new Error('p3394_session_not_found');
    const terminalTasks = (await listCogSeedTasks(userId))
      .filter((task) => task.sessionId === cogseedSessionId && TERMINAL_TASK_STATUSES.has(task.status));
    for (const task of terminalTasks) {
      try {
        await recordCogSeedTaskRunForRecall(userId, task.taskId);
      } catch (error) {
...
function mapCogSeedTaskEvent(event: CogSeedTaskEvent): Omit<P3394RuntimeEvent, 'sequence' | 'task_id'> | null {
  switch (event.type) {
    case 'task.started':
      return { kind: 'started' };
    case 'model.delta':
