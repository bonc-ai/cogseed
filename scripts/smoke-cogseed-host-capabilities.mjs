...
tsxEsm.register();
const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cogseed-host-smoke-'));
process.env.COGSEED_WORKSPACE_ROOT = root;
process.env.COGSEED_PC_DIR = pcDir;
process.env.COGSEED_COGSEED_RUNTIME_TEST_HOST_ECHO = '1';

const assert = (condition, message) => { if (!condition) throw new Error(message); };
let service;
try {
  const { createRuntimeWorkerService } = require('../src/main/features/cogseed_runtime/worker-process.ts');
  const { createCogSeedHostToolRouter } = require('../src/main/features/cogseed_backend/host-tool-router.ts');
  const calls = [];
  const router = createCogSeedHostToolRouter({
    ...
    coordinator: {
      delegate: async (_uid, _requestId, input) => { calls.push({ name: 'cogseed_delegate', task: input.task }); return { taskId: 'cogseed-task-smokechild', sessionId: 'cogseed-session-smokechild', runtimeSessionId: 'mruntime-smokechild', status: 'running' }; },
      tasks: async () => ({ coordinationId: 'cogseed-coord-smoke', children: [] }),
      cancel: async () => ({ taskId: 'cogseed-task-smokechild', status: 'cancelled' }),
    },
  });
  service = createRuntimeWorkerService({ hostToolHandler: (call, context) => router.handle(call, context) });
  const request = {
    protocol_version: 2, type: 'run', request_id: 'req-host-smoke', runtime_session_id: 'mruntime-host-smoke', user_id: 'cogseed-host-smoke-user',
    task: JSON.stringify({ hostTools: [
      { name: 'office_read', input: { path: '/tmp/report.docx' } },
      { name: 'browser_snapshot', input: {} },
      { name: 'cogseed_delegate', input: { task: 'Inspect one bounded subtask' } },
  ...
  assert(calls.map((call) => call.name).join(',') === 'office_read,browser_snapshot,cogseed_delegate', 'host calls were not ordered');
