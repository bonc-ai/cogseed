...
const RUNTIME_VARIANTS = Object.freeze(['main', 'cognition', 'expense', 'integration', 'messaging']);
...
function parseMessagingWorktreeVariant(argv) {
  const variant = parseVariant(argv);
  if (variant !== 'messaging') {
    throw new Error('this source worktree is locked to the messaging runtime variant');
  }
  return variant;
}
...
function main() {
  try {
    const result = prepareSourceRuntimeBundle({ variant: parseMessagingWorktreeVariant(process.argv.slice(2)) });
