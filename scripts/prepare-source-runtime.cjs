...
function parseCognitionWorktreeVariant(argv) {
  const variant = parseVariant(argv);
  if (variant !== 'cognition') {
    throw new Error('this source worktree is locked to the cognition runtime variant');
  }
  return variant;
}
...
function main() {
  try {
    const result = prepareSourceRuntimeBundle({ variant: parseCognitionWorktreeVariant(process.argv.slice(2)) });
