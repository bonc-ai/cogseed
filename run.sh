...
This worktree is locked to the cognition runtime identity. Run expense or
integration acceptance from their dedicated worktrees.
EOF
}
...
if [ -n "${ORKAS_RUNTIME_VARIANT:-}" ] && [ "$ORKAS_RUNTIME_VARIANT" != "cognition" ]; then
  echo "[Mate Agent] This worktree is locked to the cognition runtime; ORKAS_RUNTIME_VARIANT=$ORKAS_RUNTIME_VARIANT is not allowed." >&2
  exit 2
fi
if [ -n "${ORKAS_WORKSPACE_ROOT:-}" ]; then
  echo "[Mate Agent] This worktree manages its own cognition data root; inherited ORKAS_WORKSPACE_ROOT is not allowed." >&2
  exit 2
fi
export ORKAS_RUNTIME_VARIANT="cognition"
...
if [ "$(uname -s)" = "Darwin" ]; then
  APP_BUNDLE="$APP_DIR/node_modules/electron/dist/Mate Agent [Cognition].app"
