...
- 候选去向由用户在 App 确认面板决定：全局记忆（`memory_scope: user` → USER.md 或 `memory_scope: shared` → MEMORY.md）、分组字段区（对号入座填坑）、分组流水区（不填坑）。本技能只提供建议（`建议字段` 预选、`memory_scope` 建议），**不指定具体分组、不决定最终去向**——用户在审阅面板里逐条确认时选择。不存在"组织本体""业务本体"这类第三方层级——这个技能只服务用户个人，不做企业级**组织**路由；支持个人角色模板（学生/学者/FDE）的字段建议。
