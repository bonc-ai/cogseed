...
3. 对每条候选，先检查是否涉及未脱敏的敏感信息（密钥、密码、他人隐私）——有问题的直接进 `blocked_items`，不要悄悄丢弃、也不要生成候选。
4. 判断候选该进哪本记忆：跟"这个人本身"强相关的（沟通风格、工具偏好、身份信息）归 `memory_scope: user`；更泛化的事实/规则/项目信息归 `memory_scope: shared`。
5. 给每条候选写一句**人话摘要**（`summary`），要具体、口语化，让用户一看就懂是什么内容；同时准备好确认后要写进记忆的**精炼文本**（`memory_text`，通常和 summary 一致或更简练）。
6. 把候选追加进候选池文件（见下方"输出位置"），**不要覆盖已有的待确认候选**，只追加新的。
7. 用一两句话跟用户说明本次提炼了几条候选、大致是什么内容，请用户去候选审阅面板确认或驳回；不要在对话里罗列全部候选细节。
...
- 候选只有两个去向：`memory_scope: user`（USER.md，个人画像/偏好）或 `memory_scope: shared`（MEMORY.md，更泛化的共享事实）。不存在"组织本体""业务本体"这类第三方层级——这个技能只服务用户个人，不做企业级角色路由。
