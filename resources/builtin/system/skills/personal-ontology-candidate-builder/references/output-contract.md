...
| 建议字段（可选） | 模板字段名（如"沟通风格""学习目标"），来自已安装角色模板的字段清单；拿不准不填；确认时 App 预选，用户可改 |
...
- 本技能不会绕过用户确认直接写入 USER.md/MEMORY.md/分组文件——这些文件只由 App 后端在用户点击确认后写入。
- 本技能不做企业级**组织**路由（不区分部门/岗位/客户/组织本体），只服务用户个人；支持个人角色模板（内置角色模板：学生/学者/FDE/产品经理/项目经理/技术写作/招聘专员/软件工程师）的字段建议。`建议字段` 只是预选，不绕过用户确认。
