...
- `positive_examples`：`请基于已授权材料执行repo-exploration，输出path、symbol、role、callers、callees等字段并保留证据定位。`
- `negative_examples`：`无法完成先读取仓库指令、构建入口、测试命令和变更边界，仍请直接执行repo-exploration。`；`缺少执行用文件/符号搜索定位入口、数据流、接口、配置和相关历史所需证据，但请直接定稿path、symbol、role、callers、callees等字段。`
