---
name: AI产品规格定义
description: 把已证实结论转化为产品对象、状态机、需求、验收与验证合同，停在人工 Gate。
---

# Define｜产品对象、状态与可验收规格

本 Skill 是“个人 AI 产品方法”1+7 套件的本地 candidate。它只处理
`Define` 范围，并把输出绑定到稳定 ID、产品版本与来源引用。

`use_when`:
- 用户目标与主流程已明确，需要形成可实现规格
- 需要冻结产品对象和成功/失败状态
...
TBox：ProductObject, State, Transition, Requirement, AcceptanceCriterion。
RBox：transitions_to, satisfies, blocked_by, verified_by。
...
本阶段退出门为 `Experiment Contract Gate`，只能由规定的人类角色决定。
...
- `positive_examples`：`请基于已授权材料执行AI产品规格定义，输出结构化的可审计结果并保留证据定位。`
