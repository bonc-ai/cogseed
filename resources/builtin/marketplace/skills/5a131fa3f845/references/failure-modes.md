...
| F-DOMAIN-01 | Workflow | 来源不可访问：列出已完成部分、缺失来源和对结论的影响。 | 按原 Skill 的失败与降级规则处理 |
| F-DOMAIN-02 | Workflow | 权限或敏感度不满足：停止对应读取或外部动作，返回所需授权。 | 按原 Skill 的失败与降级规则处理 |
| F-DOMAIN-03 | Workflow | 数据互相冲突：并列冲突及适用条件，不自行裁决为单一事实。 | 按原 Skill 的失败与降级规则处理 |
| F-DOMAIN-04 | Workflow | 预算耗尽：返回当前证据、未完成步骤和可安全续跑点。 | 按原 Skill 的失败与降级规则处理 |
