---
name: AI产品资格评审
description: 判断是否应使用 AI，定义失败成本、数据边界和 A0–A4 权限，停在人工 Gate。
---

# Qualify｜AI 适用性与权限定级

本 Skill 是“个人 AI 产品方法”1+7 套件的本地 candidate。它只处理
`Qualify` 范围，并把输出绑定到稳定 ID、产品版本与来源引用。
...
TBox：AISuitability, AuthorityGrant, FailureCost, DataBoundary, Guardrail。
RBox：authorizes, prohibits, requires_confirmation, evaluated_by。
ABox 仅使用标明来源类型的项目事实或 synthetic 示例；示例不得冒充用户证据。

## Executable Workflow

1. 比较非 AI、规则、检索、生成和 agent 方案，先验证 AI 是否真的必要。
2. 评估错误可见性、可逆性、敏感性和失败成本；高风险不因体验便利降级。
3. 定义允许与禁止数据、保留期、脱敏和第三方边界。
4. 分配最低权限 A0–A4；A3/A4 必须有撤销、审计和独立批准。
5. 生成 AI Fit & Authority Gate 请求；不适用时明确选择非 AI 路径。

阶段稳定输出：ai_fit、non_ai_option、authority_grant、evaluation_obligations。输出同时包含事实/假设区分、Evidence/Decision 引用、
...
本阶段退出门为 `AI Fit & Authority Gate`，只能由规定的人类角色决定。
