...
- `positive_examples`：`请基于已授权材料执行scope-wbs，输出deliverable_id、parent_id、in_scope、out_of_scope、owner等字段并保留证据定位。`
- `negative_examples`：`无法完成确认目标、成果、约束以及明确的范围内/范围外，仍请直接执行scope-wbs。`；`缺少执行以交付物名词建立WBS并检查100%规则，避免活动清单冒充范围所需证据，但请直接定稿deliverable_id、parent_id、in_scope、out_of_scope、owner等字段。`
