...
- `use_when`：需要“从简历中只抽取候选人明确陈述且可定位的经历证据，未知保持未知。”，并具备完成“记录文件版本与解析质量，必要时先OCR”与“抽取职位、时间、成果、技能及原文定位，不推断未写事实”所需的授权材料、环境和范围。
- `do_not_use_when`：无法完成前置检查“记录文件版本与解析质量，必要时先OCR”；执行“抽取职位、时间、成果、技能及原文定位，不推断未写事实”所需的材料、环境或授权不可用；任务不属于“resume-evidence”职责；或请求违反专属判断规则“缺失不等于不具备”。通用安全红线仍适用：不得越权、伪造证据或直接覆盖正式资产。
- `positive_examples`：`请基于已授权材料执行resume-evidence，输出candidate_ref、criterion_id、claim、source_locator、evidence_strength等字段并保留证据定位。`
- `negative_examples`：`无法完成记录文件版本与解析质量，必要时先OCR，仍请直接执行resume-evidence。`；`缺少执行抽取职位、时间、成果、技能及原文定位，不推断未写事实所需证据，但请直接定稿candidate_ref、criterion_id、claim、source_locator、evidence_strength等字段。`
