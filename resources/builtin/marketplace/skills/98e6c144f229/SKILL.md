...
description: "围绕胜任力假设设计一致的核心问题、追问、工作样本和行为锚点。 用于招聘专员的专业任务；当关键证据缺失、需要越权或涉及正式写入时停止并请求确认。"
---
...
返回字段：`competency`、`hypothesis`、`core_question`、`follow_ups`、`score_anchors`、`prohibited_topics`、`timebox`。关键结论标记为事实、推断、假设、建议或待决策，并保留来源定位。只生成正式资产候选，不直接覆盖知识、计划、代码、候选人决定或客户承诺。
...
- `use_when`：需要“围绕胜任力假设设计一致的核心问题、追问、工作样本和行为锚点。”，并具备完成“把每个证据缺口转成可验证的面试假设”与“设计相同核心问题及允许的中性追问，必要时加入工作样本”所需的授权材料、环境和范围。
- `do_not_use_when`：无法完成前置检查“把每个证据缺口转成可验证的面试假设”；执行“设计相同核心问题及允许的中性追问，必要时加入工作样本”所需的材料、环境或授权不可用；任务不属于“interview-design”职责；或请求违反专属判断规则“先定义评分锚点再面试”。通用安全红线仍适用：不得越权、伪造证据或直接覆盖正式资产。
- `positive_examples`：`请基于已授权材料执行interview-design，输出competency、hypothesis、core_question、follow_ups、score_anchors等字段并保留证据定位。`
- `negative_examples`：`无法完成把每个证据缺口转成可验证的面试假设，仍请直接执行interview-design。`；`缺少执行设计相同核心问题及允许的中性追问，必要时加入工作样本所需证据，但请直接定稿competency、hypothesis、core_question、follow_ups、score_anchors等字段。`
