...
- `positive_examples`：`请基于已授权材料执行solution-design，输出outcome、capability、option、component、data_flow等字段并保留证据定位。`
- `negative_examples`：`无法完成从已确认成果与约束推导所需能力和验收信号，仍请直接执行solution-design。`；`缺少执行提出至少两个可行选项，比较构建/采购、成本、风险和可逆性所需证据，但请直接定稿outcome、capability、option、component、data_flow等字段。`
