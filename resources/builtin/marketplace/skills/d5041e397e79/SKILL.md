...
- `use_when`：需要“用固定rubric生成要求到证据的解释矩阵，暴露缺口而非自动淘汰。”，并具备完成“加载已冻结职位评分卡和合法评估范围”与“逐标准链接候选证据，标注直接/间接/无证据/矛盾”所需的授权材料、环境和范围。
- `do_not_use_when`：无法完成前置检查“加载已冻结职位评分卡和合法评估范围”；执行“逐标准链接候选证据，标注直接/间接/无证据/矛盾”所需的材料、环境或授权不可用；任务不属于“match-matrix”职责；或请求违反专属判断规则“评分只基于岗位相关证据”。通用安全红线仍适用：不得越权、伪造证据或直接覆盖正式资产。
- `positive_examples`：`请基于已授权材料执行match-matrix，输出candidate_ref、criterion_id、evidence_ref、rating、anchor_used等字段并保留证据定位。`
- `negative_examples`：`无法完成加载已冻结职位评分卡和合法评估范围，仍请直接执行match-matrix。`；`缺少执行逐标准链接候选证据，标注直接/间接/无证据/矛盾所需证据，但请直接定稿candidate_ref、criterion_id、evidence_ref、rating、anchor_used等字段。`
