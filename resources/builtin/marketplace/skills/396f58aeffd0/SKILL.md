---
name: AI产品风险排除
description: 按最大不确定性选择用户研究、原型、AI Eval、Technical Spike 或直接规格化，停在人工 Gate。
---

# De-risk｜最低成本不确定性验证

本 Skill 是“个人 AI 产品方法”1+7 套件的本地 candidate。它只处理
`De-risk` 范围，并把输出绑定到稳定 ID、产品版本与来源引用。

`use_when`:
- 体验、AI 能力、集成或价值风险仍阻断实施
- 需要冻结可判定的实验合同
...
TBox：Uncertainty, Experiment, Prototype, EvaluationSet, TechnicalSpike。
RBox：tests, reduces, passes_threshold, returns_to。
ABox 仅使用标明来源类型的项目事实或 synthetic 示例；示例不得冒充用户证据。

## Executable Workflow

1. 只选择一个当前最大不确定性，明确它阻断的产品决定。
2. 价值不确定用研究/Wizard of Oz；体验不确定用最低成本原型；AI 能力用 golden set 与离线 Eval；集成不确定用 Technical Spike；结论充分时不额外实验。
3. 实验前冻结输入、成功阈值、保护面、失败解释和停止条件。
4. 执行获准且可逆的验证，区分真实、脱敏、合成与 stub 结果。
5. 达到 Discovery Gate 标准才建议进入 Spec Kit；失败则回到负责阶段。

阶段稳定输出：validation_vehicle、experiment_contract、result、discovery_recommendation。输出同时包含事实/假设区分、Evidence/Decision 引用、
...
本阶段退出门为 `Discovery Gate`，只能由规定的人类角色决定。
自动化上限为 `staged_eligible`，当前包状态仅为 `candidate`。
不允许宣称 Level B、release ready、production ready、published、客户价值或 Level C。

原型、Spike 和 synthetic Eval 都是发现证据，不自动证明客户价值或生产可行。

<!-- SKILL-GATE:BEGIN -->
## Skill Gate 契约

- `use_when`：按最大不确定性选择用户研究、原型、AI Eval、Technical Spike 或直接规格化，停在人工 Gate。，并具备完成该任务所需的授权材料、环境和范围。
- `do_not_use_when`：所需材料、环境或授权不可用；任务不属于「AI产品风险排除」职责；或请求违反专属判断规则。通用安全红线仍适用：不得越权、不得伪造证据、不得直接覆盖正式资产。
- `positive_examples`：`请基于已授权材料执行AI产品风险排除，输出结构化的可审计结果并保留证据定位。`
- `negative_examples`：`缺少执行AI产品风险排除所需证据，仍请直接定稿。`
