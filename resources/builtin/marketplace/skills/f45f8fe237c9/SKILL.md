...
- `use_when`：把想法框定为明确的用户、场景、结果、边界和待验证问题，停在人工 Gate。，并具备完成该任务所需的授权材料、环境和范围。
- `do_not_use_when`：所需材料、环境或授权不可用；任务不属于「AI产品立项聚焦」职责；或请求违反专属判断规则。通用安全红线仍适用：不得越权、不得伪造证据、不得直接覆盖正式资产。
- `positive_examples`：`请基于已授权材料执行AI产品立项聚焦，输出结构化的可审计结果并保留证据定位。`
- `negative_examples`：`缺少执行AI产品立项聚焦所需证据，仍请直接定稿。`
