...
- Shared Renderer primitives are `uiButton(...)`, `uiIconButton(...)`, `uiInput(...)`, `uiCheckbox(...)`, `uiSwitch(...)`, `uiTextarea(...)`, `uiSelect(...)`, `uiDateRangePicker(...)`, `uiField(...)`, `uiForm(...)`, `uiEmptyState(...)`, `uiModal(...)`, `uiModalController(...)`, and `uiPageHeader(...)`; extend these seams instead of adding page-local button, form-control, empty-state, modal, or page-header variants.
