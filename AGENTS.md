...
- Shared Renderer APIs are grouped by role; extend these seams instead of adding page-local variants:
  - Actions and forms: `uiButton(...)`, `uiIconButton(...)`, `uiInput(...)`, `uiCheckbox(...)`, `uiSwitch(...)`, `uiTextarea(...)`, `uiSelect(...)`, `uiDateRangePicker(...)`, `uiField(...)`, and `uiForm(...)`.
  - Feedback and decisions: `uiEmptyState(...)`, `uiToast(...)`, `uiAlert(...)`, `uiConfirm(...)`, `uiConfirmDanger(...)`, `uiChoice(...)`, and `uiPrompt(...)`.
  - Navigation and shell: `uiSegmentedControl(...)` for compact filters, `uiTabs(...)` plus `hydrateUiTabs(...)` for same-object views, `uiPageHeader(...)`, `uiSidebarTools(...)`, `uiUserMenuTrigger(...)`, and `uiUserMenuPanel(...)`.
  - Data and status: `uiTag(...)` for non-interactive metadata, `uiChip(...)` for interactive filters, `uiBadge(...)` for compact counts or labels, `uiStatusPill(...)` and `uiStatusDot(...)` for state, `uiProgressBar(...)` for measurable work, and `uiSkeleton(...)` for unknown-duration content loading.
  - Structure: `uiCard(...)` adopts business-owned card content into the shared shell, `uiResourceCard(...)` renders the standard titled resource card, and `uiResourceGrid(...)` owns the responsive card grid.
  - Structured data: `uiDataTable(...)` for native tabular data, `uiAccordion(...)` for independent disclosure sections, and `uiTree(...)` plus `hydrateUiTrees(...)` for hierarchical navigation with shared keyboard behavior. Business-specific graph or mind-map canvases remain page-owned.
  - Overlays: `uiModal(...)` for new decision dialogs and `uiModalController(...)` when adopting existing business-owned dialog DOM; `uiDrawer(...)` for new side panels and `uiDrawerController(...)` when adopting an existing business-owned drawer without changing its data or event wiring.
  - Icons: `uiIconHtml(...)` for generated markup and `hydrateUiIcons(...)` for existing DOM placeholders.
- Renderer structural component policy uses three explicit states:
  - **Shared**: the APIs above and their production classes. Reuse them before adding business classes; page code may own only business content, routing, data and state transitions.
  - **Provisional composition**: structure without an approved shared API may remain page-local only while its role is genuinely business-specific. Such code must use tokens, native semantics and accessible keyboard behavior, must not introduce a second visual/behavioral variant, and must be proposed for the shared layer once two pages need the same stable contract.
- Page-local structure under those three states is tracked in `docs/renderer-structural-registry.md` (maintained by `@bonc-ai/reviewers`). Register a new page-local structural component there in the same PR that introduces it; mark its entry `Migrated` once a shared seam takes over instead of deleting the row; unregistered page-local structure is treated as Prohibited.
  - **Prohibited**: new raw button/form controls, page-local modal/confirm/toast shells, emoji or inline-SVG icons, literal `z-index`, or duplicated focus/keyboard behavior when a shared seam exists.
- `docs/renderer-structural-registry.md` is the authoritative decision log for Provisional and migration boundaries. New or changed page-local structural composition must reference an approved registry entry or add a `Proposed` entry in the same PR; it is not implicitly Provisional. `@bonc-ai/reviewers` owns decisions, and completed records remain in the log as `Migrated` or `Superseded` rather than being deleted.
- New or changed checkbox controls must use `uiCheckbox(...)`; do not hand-write `<input type="checkbox">` or create page-local checkbox markup. Use `uiSwitch(...)` instead when the control represents an immediate on/off setting rather than multi-select or consent semantics.
- Any Renderer feature that adds or changes a visible action, form control, feedback surface, navigation control, overlay or structural component must follow `.agents/skills/cogseed-component-change/SKILL.md` and add real-page integration coverage. `test/renderer/shared-ui-adoption-guard.test.ts` freezes the legacy raw-control baseline; do not raise that baseline to bypass migration or review.
