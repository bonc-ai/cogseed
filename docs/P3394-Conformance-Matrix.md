...
| S-04 | Audit Journal 记录可追溯事件但不泄露敏感数据 | `audit-journal.ts`、`outbound-outbox.ts`、`kstar-episodes.ts`、`executor.ts`；`redaction.test.ts`；`executor-leak.test.ts`；audit/security/auto-reply tests | 部分符合 | 已验证 artifact 自动回发审计包含 endpoint/kind/reply_to 而不写入 artifact secret；audit、outbox error 与 episode 落盘统一 canonical 脱敏并附 fixture 集；executor 异常/重试路径泄漏已封堵（deliver/onEvent 抛错带 token → episode/审计均脱敏、关联 id 可追溯）；仍需跨 Channel 的泄漏 fixture。 |
