...
7. ✅ **确定性宿主路由（治本：路由不再依赖模型自觉）**：实机两次证明模型可能空参调用/跳过 kstar_control（`input_bytes=0` 3 次失败）。改为：任务形用户消息（层 1 检测，确定性）→ **宿主直接** `upsert_state` + `request_projection`（自动确认投影）——Commander 不再需要调这两个。模型唯一 KStar 职责 = `commit_forecast`（预测本身），由门禁 + next_step 契约双保险。寒暄零写入保留（检测过滤 + env 开关 `ORKAS_KSTAR_HOST_ROUTING=0` 供测试/降级）。
