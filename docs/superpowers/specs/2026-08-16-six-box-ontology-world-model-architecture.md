...
7. ✅ **沉淀时机 = 整个任务闭环（不是每次 run）**：closure（每次 run 终态）**只复盘**——落 review + 标记 extraction-run，**不沉淀**。沉淀唯一入口是 **requirement 级聚合**（`precipitateRequirementLevel`，在 `finish`/`abandon`/新任务切换时触发）：读 requirement 全部 episode 的 review → 聚合工具链/最强信号/去重证据 → **一条资产直接入能力资产**。任务中途的 run 静止不产生碎片资产，闭环完成才沉淀完整教训。
