...
7. ✅ **任务闭环判定 = 用户下一条消息（模型判断延续 vs 新需求）**：闭环不是技术信号（会话静止/Commander finish），而是**用户使用行为**——用户满意后会**继续发新需求**（不再提旧任务），不满意会**继续发修改意见**。实现：Commander 在回复中带 `<kstar-closure>{"new_task":true|false}</kstar-closure>` 标记（模型用完整上下文判断"延续 vs 新"）；宿主 turn 后解析标记，new_task → 走 `finish` 控制路径（requirement→waiting_review + task→closing + **requirement 级沉淀**）；延续/沉默 → 不动作。用户满意→发新需求→旧任务闭环沉淀；不满意→继续改→未闭环。
