...
- 桌面应用图标版：使用 macOS 风格圆角磁贴（透明四角、圆角半径约 22%、品牌标记居中约占 62% 画布、浅色底），与其他程序坞图标尺寸和圆润度保持一致；同时生成 512px 主 PNG、ICO 多尺寸容器和 ICNS 多尺寸容器。
...
现有 `brand.json` 的产品身份字段不变。旧的 `cogseed`、`cogseed` 等协议兼容项也不变，因为它们属于内部兼容和深链接迁移，不是用户品牌图标。
