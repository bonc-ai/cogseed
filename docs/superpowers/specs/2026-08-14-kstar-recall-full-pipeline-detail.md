...
## 阶段 6：特权派发门禁 + Commander 中心化资产派发 + 终态 provenance 盖章
...
- **本体 R-Box（业务规则）做实**：本体字段值写成 `A → B` 即关系规则（`recall/ontology-rules.ts`，值形状驱动，`ontr-*` 稳定 id），进入 `simulationInput.k.ontologyRules`，与世界模型 R-Box 的资产 CausalRule（ΔR 教训）并列——本体提供持久业务规则源，资产提供经验教训源。
