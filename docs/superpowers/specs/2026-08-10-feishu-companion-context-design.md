...
在 mate-agent（Electron + TS）上实现"**个人上下文连接器框架 + 飞书首实现**"：把用户授权的飞书个人数据（日历/云空间/文档/联系人）选择性接入，经"候选实体 → 本体确认"治理后成为**个人上下文底座**，喂给可无限扩展的场景层（今日简报、日程助理、学习/研究任务、学期地图等）。
...
> 场景主体：学生（伴侣智能体使用者）。前置条件：已在 mate-agent"设置 → 消息平台"完成飞书机器人配置（QR 绑定，现有能力）。

### 2.1 首次接入（一次性，约 10 分钟）

第 2–4 步全部由**用户主动发起**，系统默认只读、只接入用户选定的资源：

1. **建立身份绑定**：学生在飞书私聊中向机器人打招呼。
   → 系统识别 `tenant + union_id`，与 mate-agent 用户建立持久绑定（双向：学生也可在桌面端发起连接，录入 `open_id` 完成绑定）。绑定完成后，机器人知道"这个飞书用户 = 这个 mate-agent 用户"。
2. **授权飞书个人数据（用户 OAuth）**：学生发起"连接我的飞书"。
   → 跳转飞书授权页，勾选只读权限（日历 / 云空间 / 文档）；回跳后 token 加密存储于机器私有区，界面显示"已连接"。
3. **选择接入资源（选择性，不扫全租户）**：学生勾选主日历（个人 + 课程）、课程资料文件夹、作品集文件夹、单个文档 / 知识库页面。
   → 系统记录接入范围（scope-manifest），做一次有限回填（近 30 天事件、未来 90 天日历），之后按游标增量同步。
4. **生成学期地图建议**：系统从已接入资源抽取候选事实（课程、截止日期、项目、师生关系等）。
   → 学生收到飞书待确认卡片，逐条批准 / 拒绝（复用现有候选确认流水线）。已批准事实进入个人本体。
5. **完成接入**：学生可随时查看"已接入资源 / 授权状态"；后续新资源接入可增量补充，无需重新走全流程。
...
- **任务委派（需批准）**：学生提出有边界的研究任务，例如"对比这三种传感器，帮我们选方案"
  → 系统先展示任务目的 + 权限范围 + 预算，等学生批准 → 在隔离工作空间执行（现有 P3394 边界，工具受限）→ 结果带引用和来源回到原飞书话题。
- **持续治理（需批准）**：系统从对话 / 文档中抽到新事实 → 飞书"待确认"卡片 → 学生批准 / 拒绝；**新会话上下文只出现已批准事实**。
- **控制权始终在手**：
...
- **端到端**（真实飞书测试租户 + 合成学生数据）：授权 → 选资源 → 回填 → 学期地图候选 → 确认 → 简报推送 → 提问召回 → 任务 Agent 委派/审批/回传 → 撤销授权后同步停止。
...
| 过度采集学生数据 | 选择性入门、只读默认、有限回填、按范围遗忘 |
