...
在 mate-agent（Electron + TS）上实现"**个人上下文连接器框架 + 飞书首实现**"：把用户授权的飞书个人数据（日历/云空间/文档/联系人）选择性接入，经"候选实体 → 本体确认"治理后成为**个人上下文底座**，喂给可无限扩展的场景层（今日简报、日程助理、学习/研究任务、学期地图等）。**定位不锁人群**：面向各行各业的个人用户，学生（学期地图、课程-截止推理）只是首个场景案例，场景层按"场景注册制"向各行业扩展，连接器与本体代码不随人群变化。
...
> 场景主体：各行各业的个人用户（学生为首个场景案例）。前置条件：已在 mate-agent"设置 → 消息平台"完成飞书机器人配置（QR 绑定，现有能力）。
...
1. **建立身份绑定**：用户在飞书私聊中向机器人打招呼。
   → 系统识别 `tenant + union_id`，与 mate-agent 用户建立持久绑定（双向：用户也可在桌面端发起连接，录入 `open_id` 完成绑定）。绑定完成后，机器人知道"这个飞书用户 = 这个 mate-agent 用户"。
