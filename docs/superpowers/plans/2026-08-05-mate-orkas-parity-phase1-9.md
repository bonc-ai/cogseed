# Mate Orkas Parity Phase 1-9 Implementation Plan
...
**Goal:** Complete the Mate-native Orkas capability parity program from Phase 1 through Phase 9 while preserving the current Runtime Worker, IPC allow-list, path sandbox, connector umbrella, and collaboration-control boundaries.

**Architecture:** Keep Mate as the canonical owner of runtime, sessions, actors, tasks, events, visibility, scheduling, tools, migration journals, and renderer projections. Use compatibility facades at the Group Chat boundary rather than importing Orkas business modules into Mate runtime code. Execute work in dependency-aware waves: independent contracts and adapters in parallel, then integrate the event/scheduler/compatibility chain serially.
...
**Files:**
- Existing: `docs/superpowers/parity/orkas-mate-capability-matrix.md`
- Existing: `docs/superpowers/parity/orkas-golden-fixture-catalog.md`
- Existing: `docs/superpowers/parity/fixtures/**/*.json`
- Existing: `scripts/capture-orkas-parity-fixtures.ts`
...
**Files:**
- Modify/Create: `src/main/features/mate_agent_backend/types.ts`
- Modify/Create: `src/main/features/mate_agent_backend/session-store.ts`
- Modify/Create: `src/main/features/mate_agent_backend/mate-control-service.ts`
- Modify/Create: `src/main/features/mate_agent_backend/collaboration-dispatcher.ts`
- Modify/Create: `src/main/features/mate_agent_backend/index.ts`
- Modify: `src/main/features/group_chat/state.ts`
- Modify: `src/main/features/group_chat/router.ts`
- Tests: `test/main/features/mate_agent_backend/*.test.ts`, `test/main/features/group_chat/state.test.ts`, `test/main/features/group_chat/router.test.ts`
...
- [ ] Implement the compatibility facade without making Mate runtime import Orkas Group Chat business logic.
...
- Modify: `src/main/features/mate_agent_runtime/kernel/tools/host-tools.ts`
- Modify: `src/main/features/mate_agent_runtime/kernel/tools/file-tools.ts`
- Modify: `src/main/features/mate_agent_runtime/kernel/tools/shell-tools.ts`
- Modify: `src/main/features/mate_agent_runtime/kernel/tools/skill-tools.ts`
- Modify: `src/main/features/mate_agent_backend/host-tool-router.ts`
- Tests: `test/main/features/mate_agent_runtime/kernel/tool-schema.test.ts`, `test/main/features/mate_agent_runtime/kernel/tool-runtime.test.ts`, `test/main/features/mate_agent_backend/host-tool-router.test.ts`

- [ ] Inventory the approved Orkas tool names against `TOOL_CATALOG` and classify missing entries.
...
**Files:**
- Modify: `src/main/features/mate_agent_backend/connector-manager.ts`
- Modify: `src/main/features/mate_agent_backend/connector-store.ts`
- Modify: `src/main/features/mate_agent_backend/mate-kb-store.ts`
- Modify: `src/main/features/mate_agent_backend/office-adapter.ts`
- Modify: `src/main/features/mate_agent_backend/browser-adapter.ts`
- Modify: `src/main/features/mate_agent_backend/browser-manager.ts`
- Modify: `src/main/features/mate_agent_backend/host-tool-router.ts`
- Tests: `test/main/features/mate_agent_backend/mate-connectors.test.ts`, `test/main/features/mate_agent_backend/mate-kb.test.ts`, `test/main/features/mate_agent_backend/office-adapter.test.ts`, `test/main/features/mate_agent_backend/browser-manager.test.ts`, `test/main/features/mate_agent_backend/host-capability-boundary.test.ts`

- [ ] Compare the approved tool catalog with Connector, KB, Office, and Browser adapters.
- [ ] Implement missing Mate-owned lifecycle and scope adapters without bypassing connector authorization, path sandboxing, or Browser/Office isolation.
...
**Files:**
- Create/Modify: `src/main/features/mate_agent_backend/visibility-policy.ts`
- Create/Modify: `src/main/features/mate_agent_backend/event-bus.ts`
- Modify: `src/main/features/collaboration_control/engine.ts`
- Modify: `src/main/features/collaboration_control/event-replay.ts`
- Modify: `src/main/features/mate_agent_backend/event-store.ts`
- Modify: `src/main/features/mate_agent_backend/collaboration-dispatcher.ts`
- Tests: `test/main/features/collaboration_control/engine.test.ts`, `test/main/features/mate_agent_backend/event-store.test.ts`, new visibility/event-bus tests
...
**Files:**
- Modify: `src/main/features/mate_agent_backend/ipc-service.ts`
- Modify: `src/main/ipc/index.ts`
- Modify: `src/main/preload.js`
- Modify: `src/renderer/index.html`
- Modify: `src/renderer/modules/conversation.js`
- Modify: `src/renderer/modules/conversation-info.js`
- Tests: `test/main/features/mate_agent_backend/ipc-service.test.ts`, `test/main/ipc/security.test.ts`, renderer collaboration and wake tests
...
- [ ] Keep `window.orkas.invoke` / `window.orkas.stream` allow-list boundaries unchanged except for explicit Mate routes.
...
- Modify: `src/main/features/mate_agent_backend/mate-execution-store.ts`
...
- Tests: `test/main/features/mate_agent_backend/coordinator.test.ts`, `coordinator-integration.test.ts`, `recovery.test.ts`, `task-store.test.ts`, new scheduler fairness and lease tests
...
**Files:**
- Modify: `src/main/features/mate_agent_backend/collaboration-dispatcher.ts`
- Modify: `src/main/features/mate_agent_backend/mate-control-service.ts`
...
- Modify: `src/main/features/mate_agent_backend/session-store.ts`
- Tests: `test/main/features/group_chat/plan-executor-terminal-delivery.test.ts`, `failed-turn-retry.test.ts`, `test/main/features/mate_agent_backend/collaboration-adapter.test.ts`, new compatibility tests
...
- Modify/Create: `src/main/features/mate_agent_backend/migration-journal.ts`
- Modify: `src/main/features/mate_agent_backend/session-store.ts`
- Modify: `src/main/features/mate_agent_backend/recovery.ts`
- Tests: `test/main/util/migrate-session-ids.test.ts`, `test/main/features/mate_agent_backend/session-store.test.ts`, new migration journal and rollback tests
...
**Files:**
- Modify: `docs/superpowers/parity/orkas-mate-capability-matrix.md`
- Create/Modify: `scripts/benchmark-mate-scheduler.mjs`
- Create/Modify: `scripts/fault-inject-mate-recovery.mjs`
- Create/Modify: `test/main/features/mate_agent_backend/surpass-audit.test.ts`
- Modify: `test/main/features/mate_agent_backend/*.test.ts`
