...
**Architecture:** Add `features/cogseed_runtime/` as the owning facade and protocol layer. Extend the existing path/session chokepoints so `mruntime-*` sessions route to `<uid>/local/cogseed_runtime/`, then run the worker through one controlled spawn helper and an injectable core-agent executor.
...
- Test: `test/main/features/cogseed_runtime/session-routing.test.ts`

- [x] **Step 1: Write failing tests**

```ts
expect(paths.cogseedRuntimeSessionFile(uid, 'mruntime-demo')).toBe(path.join(paths.userLocalRoot(uid), 'cogseed_runtime', 'sessions', 'mruntime-demo.jsonl'));
expect(resolveSessionPath(uid, 'mruntime-demo')).toBe(paths.cogseedRuntimeSessionFile(uid, 'mruntime-demo'));
expect(memoryScopeForSession('mruntime-demo', 'agent1')).toBeNull();
```

- [x] **Step 2: Run test to verify failure**

Run: `npm run test:js -- run test/main/features/cogseed_runtime/session-routing.test.ts --maxWorkers=1`
Expected: FAIL because runtime path helpers and `mruntime` session kind do not exist.

- [x] **Step 3: Implement routing**

Add path helpers under `local/cogseed_runtime/{sessions,conversations,memory,contexts,runs}` and teach `session-store.ts` that `mruntime` is a known local-only kind routed to `cogseedRuntimeSessionFile` with no cross-session memory scope.
...
**Files:**
- Create: `src/main/features/cogseed_runtime/protocol.ts`
...
Run: `npm run test:js -- run test/main/features/cogseed_runtime/protocol.test.ts --maxWorkers=1`
...
**Files:**
- Create: `src/main/features/cogseed_runtime/store.ts`
- Create: `src/main/features/cogseed_runtime/worker.ts`
- Create: `src/main/features/cogseed_runtime/worker-entry.ts`
- Create: `src/main/features/cogseed_runtime/worker-process.ts`
- Create: `bin/cogseed-runtime-worker.cjs`
- Test: `test/main/features/cogseed_runtime/store.test.ts`
- Test: `test/main/features/cogseed_runtime/worker-process.test.ts`

- [x] **Step 1: Write failing tests**

```ts
await appendRuntimeRunEvent(uid, 'run1', { type: 'event', request_id: 'req1', runtime_session_id: 'mruntime-a', status: 'started' });
expect(fs.existsSync(path.join(paths.userLocalRoot(uid), 'cogseed_runtime', 'runs', 'run1', 'events.jsonl'))).toBe(true);
...
Run: `npm run test:js -- run test/main/features/cogseed_runtime/store.test.ts test/main/features/cogseed_runtime/worker-process.test.ts --maxWorkers=1`
Expected: FAIL because store and worker process files do not exist.

- [x] **Step 3: Implement store and worker service**

Persist runtime run metadata/events under `local/cogseed_runtime/runs`; implement JSONL handshake, request/response correlation, cancel, restart after exit, and protocol version rejection in `worker-process.ts`; implement `worker.ts` with an injectable executor and an echo executor used only under `COGSEED_COGSEED_RUNTIME_TEST_ECHO=1`.
...
**Files:**
- Create: `src/main/features/cogseed_runtime/core-executor.ts`
- Create: `src/main/features/cogseed_runtime/index.ts`
- Test: `test/main/features/cogseed_runtime/facade.test.ts`

- [x] **Step 1: Write failing tests**

```ts
const runtime = createCogSeedAgentRuntime({ worker: fakeWorker, projectResult: fakeProjector });
...
Run: `npm run test:js -- run test/main/features/cogseed_runtime --maxWorkers=1`
