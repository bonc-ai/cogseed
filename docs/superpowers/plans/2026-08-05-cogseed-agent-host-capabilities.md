...
**Goal:** Add CogSeed-owned Office, Browser, and bounded Multi-Agent Coordinator capabilities to the isolated Native Runtime through a Main-side host-tool protocol.

**Architecture:** Extend the existing Runtime JSONL worker protocol with a reverse `host_tool_call`/`host_tool_result` request-response path. The Worker remains limited to the Native kernel; Main owns OfficeCLI, Electron BrowserWindow, and child-task scheduling. Each capability has a CogSeed-owned adapter, user/session scope, bounded results, cancellation, and focused tests.
...
- Modify: `src/main/features/cogseed_runtime/worker-process.ts`
- Modify: `src/main/features/cogseed_runtime/worker.ts`
- Modify: `src/main/features/cogseed_runtime/worker-entry.ts`
- Modify: `src/main/features/cogseed_runtime/kernel/types.ts`
- Modify: `src/main/features/cogseed_runtime/kernel/tools/runner.ts`
- Create: `src/main/features/cogseed_runtime/kernel/tools/host-tools.ts`
- Create: `test/main/features/cogseed_runtime/host-tools.test.ts`
- Create: `test/main/features/cogseed_runtime/worker-host-protocol.test.ts`
...
```bash
npm run test:js -- test/main/features/cogseed_runtime/host-tools.test.ts test/main/features/cogseed_runtime/worker-host-protocol.test.ts
```
...
**Files:**
- Create: `src/main/features/cogseed_backend/office-adapter.ts`
...
- Modify: `src/main/features/cogseed_backend/index.ts`
- Modify: `src/main/features/cogseed_runtime/worker-process.ts`
- Create: `test/main/features/cogseed_backend/office-adapter.test.ts`
- Create: `test/main/features/cogseed_runtime/kernel/office-tool-contract.test.ts`
...
```bash
npm run test:js -- test/main/features/cogseed_backend/office-adapter.test.ts test/main/features/cogseed_runtime/kernel/office-tool-contract.test.ts
```

Expected: FAIL because the Mate Office adapter and host catalog entries are absent.
- [ ] **Step 3: Implement `office-adapter.ts`.** Export `createCogSeedOfficeAdapter` with `run(name, input, scope, opts)`. Normalize paths before checking existence, reject unsupported extensions and malformed batch operations, call only `runOfficeCli` with argv arrays, use temporary output paths under a scoped root, and close resident files in `finally`.
...
**Files:**
- Create: `src/main/features/cogseed_backend/browser-manager.ts`
- Create: `src/main/features/cogseed_backend/browser-adapter.ts`
- Create: `src/main/features/cogseed_runtime/kernel/tools/browser-tool-contract.ts`
- Modify: `src/main/features/cogseed_runtime/kernel/tools/catalog.ts`
- Modify: `src/main/features/cogseed_backend/index.ts`
- Modify: `src/main/features/cogseed_runtime/worker-process.ts`
- Create: `test/main/features/cogseed_backend/browser-manager.test.ts`
- Create: `test/main/features/cogseed_runtime/kernel/browser-tool-contract.test.ts`
...
```bash
npm run test:js -- test/main/features/cogseed_backend/browser-manager.test.ts test/main/features/cogseed_runtime/kernel/browser-tool-contract.test.ts
```
...
```bash
npm run test:js -- test/main/features/cogseed_backend/browser-manager.test.ts test/main/features/cogseed_runtime/kernel/tools/browser-tool-contract.test.ts
npm run typecheck
```
...
- Modify: `src/main/features/cogseed_backend/paths.ts`
- Modify: `src/main/features/cogseed_backend/types.ts`
- Create: `src/main/features/cogseed_backend/coordination-store.ts`
- Create: `src/main/features/cogseed_backend/coordinator.ts`
- Modify: `src/main/features/cogseed_backend/task-store.ts`
- Modify: `src/main/features/cogseed_backend/runtime-controller.ts`
- Modify: `src/main/features/cogseed_backend/index.ts`
- Create: `test/main/features/cogseed_backend/coordinator.test.ts`
...
```bash
npm run test:js -- test/main/features/cogseed_backend/coordinator.test.ts
```

Expected: FAIL because coordination records, links, and APIs are absent.
- [ ] **Step 3: Implement the store and record shape.** Add `<uid>/cloud/cogseed/coordinations`, `cogseed-coord-*` validation, atomic JSON writes, and optional `coordinationId`, `parentTaskId`, and `coordinationDepth` on tasks. Keep ordinary task creation unchanged when no coordinator metadata is passed.
- [ ] **Step 4: Implement bounded coordinator methods.** `delegate` validates explicit task/context and creates a child through `CogSeedRuntimeController`; `tasks` reads only linked children; `cancel` rejects unlinked ids and calls `cancelCogSeedTask`. Enforce `maxChildren=4` and child `coordinationDepth=1` without wall-clock timeouts.
...
**Files:**
- Create: `src/main/features/cogseed_backend/host-tool-router.ts`
- Modify: `src/main/features/cogseed_runtime/worker-process.ts`
- Modify: `src/main/features/cogseed_backend/runtime-controller.ts`
- Modify: `src/main/features/cogseed_backend/index.ts`
- Create: `test/main/features/cogseed_backend/host-tool-router.test.ts`
- Create: `test/main/features/cogseed_backend/coordinator-integration.test.ts`
...
```bash
npm run test:js -- test/main/features/cogseed_backend/host-tool-router.test.ts test/main/features/cogseed_backend/coordinator-integration.test.ts
```

Expected: FAIL because the router and lifecycle hooks are absent.
- [ ] **Step 3: Implement `createCogSeedHostToolRouter`.** Dispatch only the allowlisted host names, pass the pending Runtime request scope into adapters, and return stable error strings through the existing result-cap utility. Use lazy imports for the shared controller to avoid Runtime ↔ Backend module cycles.
- [ ] **Step 4: Add lifecycle hooks.** On terminal Runtime events dispose Browser sessions and ask Coordinator to cancel active children when the parent task is cancelled. Do not create duplicate task/event scheduling paths; all tasks go through `CogSeedRuntimeController` and all events go through `appendCogSeedTaskEvent`.
- [ ] **Step 5: Run the focused backend/runtime suite.**

```bash
npm run test:js -- test/main/features/cogseed_backend test/main/features/cogseed_runtime test/main/ipc/cogseed-agent-backend.test.ts
npm run typecheck
```
...
**Files:**
- Modify: `scripts/smoke-cogseed-agent-native.mjs`
- Create: `scripts/smoke-cogseed-agent-host-capabilities.mjs`
- Create: `test/main/features/cogseed_backend/host-capability-boundary.test.ts`
...
git diff --check
npm run typecheck
npm run test:js -- test/main/features/cogseed_runtime test/main/features/cogseed_backend test/main/ipc/cogseed-agent-backend.test.ts
npm test
...
- [ ] **Step 4: Audit changes.** Confirm no new npm dependency, no new HTTP server, no direct CogSeed Group Chat/Core business import, no raw browser storage in results, and no files outside CogSeed-owned paths. Record any environment-specific limitation rather than claiming support.
