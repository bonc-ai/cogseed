...
import {
  ...
  classifyCognitionSensitivityOfParts,
...
describe('多段一起过闸', () => {
  it('凭证藏在 summary 里也拦得住', () => {
    // 把凭证写在次要字段里绕过主字段检查，是最容易想到的规避方式。
    const verdict = classifyCognitionSensitivityOfParts([
      '一条看起来很正常的判断。',
  ...
  it('全部正常时放行', () => {
    expect(classifyCognitionSensitivityOfParts(['正常判断', '正常摘要', undefined]).level)
      .toBe('unclassified');
  });
