...
const sourceRuntime = require('../../../scripts/prepare-source-runtime.cjs') as {
  ...
  parseCognitionWorktreeVariant(argv: readonly string[]): string;
};
...
function createCurrentBundleFixture() {
  ...
  const destination = path.join(root, 'Mate Agent [Cognition].app');
  const identity = sourceRuntime.sourceRuntimeBundleSpec('cognition');
...
describe('macOS source runtime bundle contract', () => {
  ...
  it('requires exactly one canonical, case-sensitive preparation variant', () => {
    ...
    expect(() => sourceRuntime.parseVariant(['--variant=Cognition'])).toThrow('invalid');
    expect(() => sourceRuntime.parseVariant(['--variant=main', '--variant=cognition'])).toThrow('exactly one');
  });

  it('locks the executable bundle-preparation entry to cognition', () => {
    expect(sourceRuntime.parseCognitionWorktreeVariant(['--variant=cognition'])).toBe('cognition');
    expect(() => sourceRuntime.parseCognitionWorktreeVariant(['--variant=integration']))
      .toThrow('locked to the cognition runtime');
  });
