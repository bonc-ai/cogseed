...
describe('runtime variant isolation', () => {
  it('locks every direct source entry to this worktree identity', () => {
    expect(RUNTIME_VARIANTS).toEqual(['main', 'cognition', 'expense', 'integration', 'messaging', 'optimization']);
    expect(installRoot.SOURCE_RUNTIME_VARIANTS).toEqual(['cognition', 'expense', 'integration', 'messaging', 'optimization']);
