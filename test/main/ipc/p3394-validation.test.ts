...
beforeEach(async()=>{root=fs.mkdtempSync(path.join(os.tmpdir(),'p3394-val-ipc-'));process.env.ORKAS_WORKSPACE_ROOT=root;invokeHandler=null;vi.resetModules();vi.doMock('../../../src/main/ipc/local_agents',()=>({invokeHandlers:{}}));const u=await import('../../../src/main/features/users');u.activateUser(UID);(await import('../../../src/main/ipc/index')).register();});
afterEach(()=>{delete process.env.ORKAS_WORKSPACE_ROOT;fs.rmSync(root,{recursive:true,force:true});vi.resetModules();});
