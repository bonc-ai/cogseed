...
beforeEach(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cogseed-skillreg-'));
...
describe('skill-registry › getSystemPromptBlock(allowlist)', () => {
  ...
  it('reuses agent-private SkillLoader instances while still seeing root mtime changes', async () => {
    ...
    const { getSystemPromptBlock } = await loadRegistry();
    const first = await getSystemPromptBlock({ agentId: 'sample-agent' });

    expect(first).toContain('private-one');
    expect(constructed).toBe(2); // trusted loader + one agent-private root loader

    writeSkill(agentPrivateDir('sample-agent'), 'private-two', 'private-two', 'second private skill');
    const later = new Date(Date.now() + 2000);
    fs.utimesSync(agentPrivateDir('sample-agent'), later, later);
    const second = await getSystemPromptBlock({ agentId: 'sample-agent' });
  ...
  it('lists agent-owned private and self-evolved skill ids', async () => {
    ...
    await expect(listAgentOwnedSkillIds(TEST_UID, 'sample-agent')).resolves.toEqual([
      'private-one',
      'evolved-one',
    ]);
  });
