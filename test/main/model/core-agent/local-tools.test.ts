...
describe('local-tools › direct CLI › script progress scanner', () => {
  ...
  it('parses progress JSONL lines, including across chunk boundaries', async () => {
    ...
    const first = scanner.feed('{"type":"progress","source":"script_worker","op":"trim","status":"encoding"}\n{"type":"prog');
    expect(first).toHaveLength(1);
    expect(first[0]).toMatchObject({ source: 'script_worker', op: 'trim', status: 'encoding' });
    // Second half of the split line arrives in the next chunk.
    const second = scanner.feed('ress","source":"audio_worker","op":"transcribe","status":"heartbeat"}\n');
    expect(second).toHaveLength(1);
    expect(second[0]).toMatchObject({ source: 'audio_worker', op: 'transcribe' });
  });
  ...
  it('ignores non-progress stderr content and look-alike lines', async () => {
    ...
    const events = scanner.feed([
      ...
      '{"type":"progress","source":"script_worker","status":"heartbeat"}',
      '',
    ].join('\n') + '\n');
    expect(events).toHaveLength(1);
    expect(events[0]).toMatchObject({ type: 'progress', source: 'script_worker' });
  });
  ...
  it('formats a readable one-liner and prefers an explicit message', async () => {
    ...
    const line = formatScriptProgress({ source: 'script_worker', op: 'trim', status: 'encoding', percent: 42.4, out_time_sec: 12.6 });
    expect(line).toContain('script_worker');
