...
describe("P3394 wake service", () => {
  it("persists and reuses a pending wake request when the agent has no approval", async () => {
    ...
    const first = await wake.evaluateWake(uid, {
      ...
      sourceActorId: "user",
      objective: "检查这个项目",
      dispatchPayload: { text: "@软件工程师 检查这个项目", asset_ids: ["ast-a", "ast-b"] },
    });
    ...
    expect(requests[0]).toMatchObject({
      ...
      status: "pending",
      objective: "检查这个项目",
      // ability assets granted pre-approval must survive to the approved
      // dispatch, otherwise a wake-gated multi-agent handoff loses them.
      dispatch_payload: { text: "@软件工程师 检查这个项目", asset_ids: ["ast-a", "ast-b"] },
    });
  });
  ...
  it("does not drop an explicit asset grant when merging an equivalent pending request", async () => {
    const wake =
      await import("../../../../src/main/features/p3394/wake-service");
    ...
    const second = await wake.evaluateWake(uid, {
      ...
      sourceActorId: "commander",
      objective: "同一个任务",
      dispatchPayload: { text: "同一个任务", asset_ids: ["asset-a", "asset-a"] },
    });

    expect(second.request.id).toBe(first.request.id);
    expect(second.request.dispatch_payload.asset_ids).toEqual(["asset-a"]);
  });
