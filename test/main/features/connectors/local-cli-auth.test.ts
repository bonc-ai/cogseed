...
describe('local CLI authorization', () => {
  it('skips the login entirely when the CLI already holds a session', async () => {
    ...
    mocks.callTool = vi.fn(async (name: string) => {
      calls.push(name);
      if (name === 'check_login') return { logged_in: true, user_name: 'Example User' };
      return {};
    });
    ...
    expect(result).toEqual({ user_name: 'Example User' });
  ...
  it('opens the browser AND publishes the URL, then resolves once the poll reports a session', async () => {
    ...
    mocks.callTool = vi.fn(async (name: string) => {
      ...
      if (name === 'check_login') {
        polls += 1;
        return { logged_in: polls > 1, user_name: polls > 1 ? 'Example User' : '' };
      }
      if (name === 'start_login') {
  ...
  it('still succeeds when the browser refuses to open, because the URL was published first', async () => {
    ...
    mocks.callTool = vi.fn(async (name: string) => {
      if (name === 'check_login') {
        polls += 1;
        return { logged_in: polls > 1, user_name: 'Example User' };
      }
    ...
    const mod = await load();
    await expect(mod.ensureLocalCliAuthorized(ENTRY as any)).resolves.toEqual({ user_name: 'Example User' });
