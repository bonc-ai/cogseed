...
describe('memory › per-agent scope', () => {
  it('routes user / shared / agent writes to separate stores that do not bleed', async () => {
    ...
    mem.addEntry('u1', { agent: 'sample-agent' }, 'plan.json is the EDL');
    ...
    expect(mem.listEntries('u1', { agent: 'sample-agent' }).entries).toEqual(['plan.json is the EDL']);
  ...
  it('agent stores have their own char budget (a busy agent cannot evict another agent / shared)', async () => {
    ...
    mem.addEntry('u1', { agent: 'sample-agent' }, big);
...
describe('memory › formatForSystemPrompt assembly', () => {
  it('an agent sees user + shared + ONLY its own notes, never another agent\'s', async () => {
    ...
    mem.addEntry('u1', { agent: 'sample-agent' }, 'video fact');
    mem.addEntry('u1', { agent: 'seo-geo' }, 'seo fact');

    const vs = mem.formatForSystemPrompt('u1', 'sample-agent');
  ...
  it('merges legacy agent-dir memory with the shared agent memory scope without duplicating text', async () => {
    const mem = await loadMemory();
    const legacyFile = path.join(tmpDir, 'u1', 'cloud', 'agents', 'sample-agent', 'memory', 'MEMORY.md');
    const canonicalFile = path.join(tmpDir, 'u1', 'cloud', 'memory', 'agents', 'sample-agent', 'MEMORY.md');
    fs.mkdirSync(path.dirname(legacyFile), { recursive: true });
    fs.writeFileSync(legacyFile, 'keeps concise endings', 'utf8');
    mem.addEntry('u1', { agent: 'sample-agent' }, 'keeps concise endings');
    mem.addEntry('u1', { agent: 'sample-agent' }, 'checks final artifact paths');

    const block = mem.formatForSystemPrompt('u1', 'sample-agent');
  ...
  it('migrates legacy agent memory once and does not keep re-reading the legacy path', async () => {
    ...
    expect(mem.formatForSystemPrompt('u1', 'sample-agent')).toContain('legacy-only note');
    expect(fs.readFileSync(legacyFile, 'utf8')).toBe('');
  ...
  it('updates and removes migrated legacy agent memory through the canonical store', async () => {
    ...
    const updated = mem.replaceAgentEntry('u1', 'sample-agent', 'old preference', 'new preference');
    expect(updated.ok).toBe(true);
    expect(updated.entries).toEqual(['new preference']);
    expect(fs.readFileSync(canonicalFile, 'utf8')).toContain('new preference');
    expect(fs.readFileSync(legacyFile, 'utf8')).toBe('');

    const removed = mem.removeAgentEntry('u1', 'sample-agent', 'new preference');
    expect(removed.ok).toBe(true);
    expect(removed.entries).toEqual([]);
    expect(mem.formatForSystemPrompt('u1', 'sample-agent')).toBe('');
  });
