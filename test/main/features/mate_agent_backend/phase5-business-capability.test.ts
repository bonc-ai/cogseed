...
describe('Phase 5 business capability parity', () => {
  it('enforces connector actor/session scope and caps oversized connector results', async () => {
    ...
    const { createMateConnectorManager } = await import('../../../../src/main/features/mate_agent_backend/connector-manager');
  ...
  it('exposes KB list/search/read compatibility semantics with source scope and bounded chunks', async () => {
    const { createMateKbManager } = await import('../../../../src/main/features/mate_agent_backend/mate-kb-store');
  ...
  it('provides specialized Office facades and registers created output plus previews', async () => {
    const { createMateOfficeAdapter } = await import('../../../../src/main/features/mate_agent_backend/office-adapter');
    ...
    const { mateCapabilityArtifactRegistry } = await import('../../../../src/main/features/mate_agent_backend/capability-artifact-lifecycle');
  ...
  it('rejects unsafe post-click navigation and cleans auto browser artifacts on dispose', async () => {
    const { createMateBrowserManager } = await import('../../../../src/main/features/mate_agent_backend/browser-manager');
