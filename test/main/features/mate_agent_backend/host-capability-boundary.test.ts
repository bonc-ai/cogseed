...
const mateFiles = [
  path.join(ROOT, 'features/mate_agent_backend/office-adapter.ts'),
  path.join(ROOT, 'features/mate_agent_backend/browser-manager.ts'),
  path.join(ROOT, 'features/mate_agent_backend/coordinator.ts'),
  path.join(ROOT, 'features/mate_agent_backend/host-tool-router.ts'),
];
...
describe('Mate host capability boundaries', () => {
  ...
  it('keeps Runtime worker spawn in the approved choke point', () => {
    const runtimeDir = path.join(ROOT, 'features/mate_agent_runtime');
