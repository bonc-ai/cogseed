...
describe("group_chat bus › p3394 envelope carriage (translator phase 1)", () => {
  it("enqueue 携带的 p3394_envelope 挂到派发 item 的 sourceP3394Envelope", async () => {
    const cid = newCid();
    const state = await import(
      "../../../../src/main/features/group_chat/state"
    );
    const bus = await import(
      "../../../../src/main/features/group_chat/bus"
    );
    _setScript(state.buildGconvSessionId(cid), [
      { type: "final", text: "ok" },
    ]);
    const items: any[] = [];
    bus._setActorTurnPreBodyHookForTest(async (_rt, _actor, item) => {
      items.push(item);
    });
    try {
      await bus.enqueue({
        ...
        text: "carry envelope",
        p3394_envelope: {
          message_id: "msg_carrier_1",
          idempotency_key: "inst-x:m-1",
        },
      } as any);
      await waitForQuiescent(TEST_UID, cid, 5000);
    } finally {
    ...
    expect(items[0].sourceP3394Envelope).toMatchObject({
      message_id: "msg_carrier_1",
      idempotency_key: "inst-x:m-1",
    });
  });

  it("不传 p3394_envelope 时 item 无该字段，行为与现状一致", async () => {
    const cid = newCid();
    const state = await import(
      "../../../../src/main/features/group_chat/state"
    );
    ...
    expect(items.length).toBeGreaterThan(0);
    expect(items[0].sourceP3394Envelope).toBeUndefined();
  });
});
