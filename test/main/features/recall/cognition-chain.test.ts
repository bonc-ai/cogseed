...
describe('认知履历：摘要用的计数', () => {
  ...
  it('数出进了几个智能体，并给出是哪几个', async () => {
    ...
    await recordAgentInheritance(UID, { agentId: 'ag-one', rolePrompt: '角色一', assets: [asset], createdAt: new Date().toISOString() });
    await recordAgentInheritance(UID, { agentId: 'ag-two', rolePrompt: '角色二', assets: [asset], createdAt: new Date().toISOString() });
