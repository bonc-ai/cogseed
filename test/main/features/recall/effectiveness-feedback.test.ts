...
async function modules() { const [candidates, refs, projection, proofs, feedback] = await Promise.all([import('../../../../src/main/features/recall/candidate-service'), import('../../../../src/main/features/recall/workspace-refs'), import('../../../../src/main/features/recall/context-projection'), import('../../../../src/main/features/recall/proof-service'), import('../../../../src/main/features/recall/effectiveness-feedback')]); return { candidates, refs, projection, proofs, feedback }; }
async function successfulTransfer() { const { candidates, refs, projection, proofs } = await modules(); const candidate = await candidates.saveRecallCandidate('user-a', { judgment: 'Record decision evidence.', suggestedType: 'rule', suggestedScope: 'review', sourceRefs: [{ kind: 'execution', id: 'exec-a' }] }); const { asset } = await candidates.promoteRecallCandidate('user-a', candidate.id); await refs.addWorkspaceAssetReference('user-a', { assetId: asset.id, workspaceId: 'workspace-a', scope: 'review' }); const preview = await projection.previewContextProjection('user-a', { taskRunId: 'task-a', workspaceId: 'workspace-a', purpose: 'review' }); const confirmed = await projection.confirmContextProjection('user-a', preview.id); const prepared = await proofs.prepareTransferProof('user-a', { projectionId: confirmed.id, executionId: 'run-a', expectedResultSnapshot: 'Expected result.' }); await proofs.completeTransferProof('user-a', prepared.id, { status: 'succeeded', observedTransfer: 'done' }); return prepared; }

async function successfulAttemptTransfer() {
  const { candidates, refs, projection, proofs } = await modules();
  const candidate = await candidates.saveRecallCandidate('user-a', { judgment: 'Keep retry attempts on the same confirmed projection.', suggestedType: 'rule', suggestedScope: 'review', sourceRefs: [{ kind: 'execution', id: 'exec-attempt' }] });
...
describe('Recall effectiveness feedback', () => {
  it('maps user feedback to effectiveness proof outcomes', async () => {
    const transfer = await successfulTransfer();
    ...
    expect(positive).toMatchObject({ outcome: 'better', status: 'valid', observedResult: 'This improved the result.' });
