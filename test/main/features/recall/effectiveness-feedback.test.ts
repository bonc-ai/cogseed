...
async function successfulAttemptTransfer() {
  ...
  const candidate = await candidates.saveRecallCandidate('user-a', { judgment: 'Keep retry attempts on the same confirmed projection.', suggestedType: 'rule', ...RULE_BOUNDARY, suggestedScope: 'review', spaceId: 'workspace-a', sourceRefs: [{ kind: 'execution', id: 'exec-attempt' }] });
