...
const runtimeEntryFiles = [
  ...
  path.join(ROOT, 'features/cogseed_backend/cogseed-control-service.ts'),
...
const { runtimeControllerLoadCount, runtimeControllerMock } = vi.hoisted(() => ({
  ...
  runtimeControllerMock: {
    startCogSeedTask: vi.fn(async () => ({ taskId: 'cogseed-task-lazy', status: 'running' })),
    cancelCogSeedTask: vi.fn(async () => ({ taskId: 'cogseed-task-lazy', status: 'cancelled' })),
    retryCogSeedTask: vi.fn(async () => ({ taskId: 'cogseed-task-lazy', status: 'running' })),
    resumeCogSeedTask: vi.fn(async () => ({ taskId: 'cogseed-task-lazy', status: 'running' })),
    runtimeStatus: vi.fn(async () => ({ backend: 'cogseed', activeTaskCount: 0, activeTaskIds: [] })),
...
vi.mock('../../../../src/main/features/cogseed_backend/runtime-controller', () => {
  ...
  return {
    cogseedRuntimeController: runtimeControllerMock,
    createCogSeedRuntimeController: vi.fn(() => runtimeControllerMock),
  };
});
...
describe('CogSeed host capability boundaries', () => {
  ...
  it('does not import CogSeed Core Office, local tools, or Group Chat business modules', () => {
    const source = cogseedFiles.map((file) => fs.readFileSync(file, 'utf8')).join('\n');
  ...
  it('does not statically import the CogSeed runtime controller from host capability entrypoints', () => {
    for (const file of runtimeEntryFiles) {
      ...
      expect(source, path.relative(ROOT, file)).not.toMatch(/import\s+\{[^}]*\bcogseedRuntimeController\b/);
    }
  });
  ...
  it('imports host capability entrypoints without initializing the CogSeed runtime controller singleton', async () => {
    ...
    await import('../../../../src/main/features/cogseed_backend/cogseed-control-service');
  ...
  it('imports cogseed-control-service without initializing the CogSeed runtime controller singleton', async () => {
    await import('../../../../src/main/features/cogseed_backend/cogseed-control-service');
    expect(runtimeControllerLoadCount.value).toBe(0);
  });
  ...
  it('resolves the default IPC runtime controller lazily at first controller-backed method call', async () => {
    ...
    expect(runtimeControllerLoadCount.value).toBe(0);
    await expect(service.runtimeStatus('ipc-user')).resolves.toEqual({ backend: 'cogseed', activeTaskCount: 0, activeTaskIds: [] });
