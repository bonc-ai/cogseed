...
import { createCogSeedCollaborationStore } from '../../../../src/main/features/cogseed_backend/collaboration-store-adapter';

const UID = 'cogseed-collab-store-user';
...
it('persists control-plane state only inside the CogSeed coordination domain', async () => {
  const store = createCogSeedCollaborationStore(); const scope = { ownerId: UID, domain: 'cogseed' as const, scopeId: 'cogseed-coord-test' };
  const run: any = { version: 1, id: 'run-1', cid: scope.scopeId, objective: 'x', kind: 'custom', status: 'running', phase: 'x', steps: [], context_id: 'ctx-1', created_by: 'cogseed', created_at: 't', updated_at: 't' };
  ...
  expect(fs.existsSync(paths.cogseedAgentCoordinationsDir(UID))).toBe(true);
