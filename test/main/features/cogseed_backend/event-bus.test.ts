...
describe('cogseed_backend append-first event bus primitives', () => {
  it('assigns append-first sequence numbers and replays in append order', async () => {
    const bus = await import('../../../../src/main/features/cogseed_backend/event-bus');
  ...
  it('can combine replay with a visibility predicate without touching storage', async () => {
    const bus = await import('../../../../src/main/features/cogseed_backend/event-bus');
    const vis = await import('../../../../src/main/features/cogseed_backend/visibility-policy');
    const store = bus.createEventBus();
