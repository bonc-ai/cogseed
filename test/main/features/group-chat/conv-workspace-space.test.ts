...
describe('conv_workspace › 空间会话工作区按空间分开存放', () => {
  it('空间会话 → 空间工作区目录（spaces/<sid>/workspace/<slug>），未绑会话 → userWorkSpace', async () => {
    ...
    // 空间会话：spaces/<空间名>/workspace/任务甲（目录名跟随空间名）
    expect(boundPath).toBe(path.join(await spaceDirFor(created.space.space_id), 'workspace', '任务甲'));
  ...
  it('已有空间会话工作区从 userWorkSpace 惰性迁移到空间目录', async () => {
    ...
    const expected = path.join(await spaceDirFor(sid), 'workspace', '旧任务');
...
describe('conv_workspace › 方案 Y：归属变更工作区随迁（不丢文件）', () => {
  ...
  it('换空间 A→B → 工作区从 A 迁到 B（文件不丢）', async () => {
    ...
    expect(p).toBe(path.join(await spaceDirFor(sidB), 'workspace', '任务'));
    expect(fs.existsSync(path.join(p, '成果.docx'))).toBe(true);
    // 旧空间目录已搬空
    expect(fs.existsSync(path.join(await spaceDirFor(sidA), 'workspace', '任务', '成果.docx'))).toBe(false);
  });
  ...
  it('删除空间 → 工作区迁到 userWorkSpace，空间目录删后文件仍在（会话保留产物）', async () => {
    ...
    const dirBefore = await spaceDirFor(sid);
