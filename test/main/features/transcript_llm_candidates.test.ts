...
const allowed = ['SpeakerA', 'Cogseed', 'KSTAR'];
...
describe('受约束候选：白名单是硬边界', () => {
  it('白名单外的目标一律丢弃，并如实记录模型编了什么', () => {
    ...
    expect(candidates.map((c) => c.correct).sort()).toEqual(['Cogseed', 'SpeakerA']);
...
describe('请求构造与跳过条件', () => {
  ...
  it('注入 runModel 时能跑通闭环', async () => {
    ...
    expect(result.candidates[0]).toMatchObject({ wrong: 'roadmap', correct: 'SpeakerA', pending: false, start: 100 });
  });
});
