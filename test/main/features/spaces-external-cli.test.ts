...
const TEST_UID = 'uGwCli';

beforeEach(async () => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cogseed-spaces-external-cli-'));
...
describe('spaces › 网关外接 CLI 字段（创建默认值）', () => {
...
describe('spaces › 网关外接 CLI 字段（旧数据兼容读）', () => {
...
describe('spaces › 网关外接 CLI 字段（update）', () => {
...
describe('spaces › 网关外接 CLI 字段（list 透传）', () => {
