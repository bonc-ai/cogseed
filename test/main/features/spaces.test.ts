...
describe('spaces › resolveSpaceResources（纯函数）', () => {
  it('模板 bundle ∪ 空间 extra 并集、去重、保序', () => {
    ...
    expect(r.template?.templateId).toBe('student');
  ...
  it('多模板（主+副）：主+副 bundle ∪ extra 并集去重', () => {
    ...
    expect(r.secondary_templates[0].templateId).toBe('scholar');
...
describe('spaces › CRUD', () => {
  it('create → list → get → update → delete 全链路', async () => {
    ...
    expect(created.space.system_name_key).toBeUndefined();
    ...
    expect(renamed.space.system_name_key).toBeUndefined();
  ...
  it('system default name key persists until a user rename clears it', async () => {
    const spaces = await loadSpaces();
    ...
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    expect(created.space.system_name_key).toBe('onboarding.temporary_space');

    const updated = await spaces.updateSpace(TEST_UID, created.space.space_id, { name: '我的空间' });
    expect(updated.ok).toBe(true);
    if (!updated.ok) return;
    expect(updated.space.name).toBe('我的空间');
    expect(updated.space.system_name_key).toBeUndefined();
  });

  it('rejects arbitrary system name keys instead of persisting them', async () => {
    const spaces = await loadSpaces();
    const created = await spaces.createSpace(TEST_UID, {
      name: '普通空间',
      system_name_key: 'user.content.name',
    });
    expect(created.ok).toBe(true);
    if (!created.ok) return;
    expect(created.space.system_name_key).toBeUndefined();
  ...
  it('persists system name keys only for a matching built-in default name', async () => {
    const spaces = await loadSpaces();
    const mismatched = await spaces.createSpace(TEST_UID, {
      name: '客户项目',
      system_name_key: 'ws.scenario.workplace.name',
    });
    expect(mismatched.ok).toBe(true);
    if (!mismatched.ok) return;
    expect(mismatched.space.system_name_key).toBeUndefined();

    const unknown = await spaces.createSpace(TEST_UID, {
      name: '普通空间',
      system_name_key: 'ws.scenario.not_a_real_scenario.name',
    });
    expect(unknown.ok).toBe(true);
    if (!unknown.ok) return;
    expect(unknown.space.system_name_key).toBeUndefined();
  });

  it('does not create duplicate Space names across localized system defaults', async () => {
    const spaces = await loadSpaces();
    const chinese = await spaces.createSpace(TEST_UID, {
      name: '职场',
      system_name_key: 'ws.scenario.workplace.name',
    });
    ...
    const english = await spaces.createSpace(TEST_UID, {
      name: 'Workplace',
      system_name_key: 'ws.scenario.workplace.name',
    });
    expect(english.ok).toBe(false);
    if (!english.ok) expect(english.error).toBe('name_dup');

    const userAlias = await spaces.createSpace(TEST_UID, { name: 'Workplace' });
    expect(userAlias.ok).toBe(false);
    if (!userAlias.ok) expect(userAlias.error).toBe('name_dup');
  });
...
describe('spaces › listSpaces 失效数（真实有效集合，假阳性回归）', () => {
