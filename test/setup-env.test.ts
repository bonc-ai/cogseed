...
// setup ever honours an inherited `COGSEED_WORKSPACE_ROOT` again, tests start
// clobbering live data instead of a throwaway dir.
//
// This canary bites hardest exactly where the bug bit: a process spawned by
// CogSeed, which exports `COGSEED_WORKSPACE_ROOT=~/.cogseed/data` to its children.
describe('vitest workspace-root isolation', () => {
  it('freezes WS_ROOT inside the OS tmp dir, never a real data root', () => {
    ...
    expect(path.basename(WS_ROOT)).toMatch(/^cogseed-vitest-/);
  });
  ...
  it('overrides an inherited env value rather than falling back to it', () => {
    ...
    // whatever it inherited (`~/.cogseed/data` when launched from the app).
    expect(process.env.COGSEED_WORKSPACE_ROOT).toBe(WS_ROOT);
    expect(WS_ROOT).not.toMatch(/\.cogseed[/\\]data$/);
  });
