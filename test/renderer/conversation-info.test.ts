...
describe('ConversationInfo files tab', () => {
  ...
  it('shows a file that is both produced and in workspace in both sections, with a produced tag', async () => {
    const result = await renderFilesResult({
      history: [
    ...
    // 产物会同时出现在「产出」和「工作区」两个区块（各一条 data-file-path）。
    expect((result.html.match(/data-file-path=/g) || []).length).toBe(2);
    // 工作区里的产物带「产出」标记。
    expect(result.html).toContain('conversation-info-file-produced-tag');
