...
describe('kb-quiz 契约', () => {
  it('逐题作答：选项只标选中，不在点击时judge；由「下一个/查看结果」推进', () => {
    const src = source();
    expect(src).toContain("btn.className = 'kb-qz-option' + (normText(opt) === normText(given) ? ' is-selected' : '')");
    expect(src).toContain('function _submitCurrent()');
    expect(src).toContain("btn.className = 'kb-qz-option' + (normText(opt) === normText(given) ? ' is-selected' : '')");
    expect(src).toContain('_state.answers[q.id] = String(opt);');
    // 点击选项不得直接写对错状态：对错只在结果页/自评里出现
    const optionHandler = src.slice(src.indexOf("btn.addEventListener('click', () => {"), src.indexOf("list.appendChild(btn);"));
    expect(optionHandler).not.toContain('is-right');
    expect(optionHandler).not.toContain('is-wrong');
  });
  ...
  it('评分（优质/劣质）走 kb.quiz.feedback，只记本机台账', () => {
    const src = source();
  ...
  it('结果页两个动作分开：再测一次（同一套题）vs 生成后续测验（同源新题）', () => {
    const src = source();
    expect(src).toMatch(/function _retest\(\)[\s\S]{0,400}_state\.answers = \{\}/);
  ...
  it('顶部：标题、查看 N 个来源、放大、更多（复制/导出）、关闭；分享如实标待开发', () => {
    const src = source();
    ...
    expect(src).toContain("_iconButtonHtml('kb-qz-share'");
    expect(src).toContain('id="kb-qz-sources-chip"');
    expect(src).toContain('data-qz-menu="copy"');
    expect(src).toContain('data-qz-menu="export"');
    expect(src).toContain("_tr('kb.quiz.share_soon'");
    expect(src).toContain('kb-qz-soon-chip');
  });
  ...
  it('Esc 关闭、点遮罩关闭、i18n-change 重刷', () => {
    const src = source();
    expect(src).toMatch(/e\.key === 'Escape'[\s\S]{0,600}close\(\)/);
    expect(src).toMatch(/if \(e\.target === el\) \{ close\(\); return; \}/);
  ...
  it('新模块零裸控件（shared-ui-adoption-guard 对新增文件额度为 0）', () => {
    ...
    // 控件来自共享原语
    expect(source()).toContain('window.uiButton');
