...
describe('conversation actor placeholder consume', () => {
  ...
  it('does not let an uncorrelated polled row consume a live same-actor turn', () => {
    ...
    const active = new FakePlaceholder({ fromActor: 'sample-agent', turnId: 'turn-live' });
    placeholders.set(key('cid-1', 'sample-agent', 'turn-live'), active);
    ...
    expect(placeholders.get(key('cid-1', 'sample-agent', 'turn-live'))).toBe(active);
  });
  ...
  it('claims a genuine boot interruption only from its actor-only legacy placeholder', () => {
    ...
    const interrupted = new FakePlaceholder({ fromActor: 'sample-agent' });
    const resumed = new FakePlaceholder({ fromActor: 'sample-agent', turnId: 'turn-live' });
    placeholders.set(key('cid-old', 'sample-agent'), interrupted);
    placeholders.set(key('cid-live', 'sample-agent', 'turn-live'), resumed);
    const status = {
      id: 'interruption-row',
      from: 'sample-agent',
      system_kind: 'reply_interrupted',
    ...
    expect(placeholders.get(key('cid-live', 'sample-agent', 'turn-live'))).toBe(resumed);
  });

  it('claims a polled terminal row only for its exact persisted turn id', () => {
    const { placeholders, consumeHistory, key } = loadPlaceholderHelpers();
    const old = new FakePlaceholder({ fromActor: 'sample-agent', turnId: 'turn-old' });
    const active = new FakePlaceholder({ fromActor: 'sample-agent', turnId: 'turn-live' });
    placeholders.set(key('cid-1', 'sample-agent', 'turn-old'), old);
    placeholders.set(key('cid-1', 'sample-agent', 'turn-live'), active);
...
describe('conversation interruption bubble collapse', () => {
  ...
  it('removes a superseded same-actor interruption without crossing a user message', () => {
    ...
    const interruption = { id: 'status', from: 'sample-agent', system_kind: 'reply_interrupted' };
    const resumed = { id: 'answer', from: 'sample-agent', turn_id: 'turn-2' };
  ...
  it('keeps exactly one SampleAgent bubble through stale polling, resumed progress, and terminal persistence', () => {
    const { placeholders, consumeHistory, key } = loadPlaceholderHelpers();
    ...
    placeholders.set(key('cid-video', 'sample-agent', 'video-turn-live'), live);
    ...
    expect(placeholders.get(key('cid-video', 'sample-agent', 'video-turn-live'))).toBe(live);
  ...
  it('does not append a false interruption below a currently running SampleAgent bubble', () => {
    const { placeholders, consumeHistory, deferHistory, key } = loadPlaceholderHelpers();
    const { removeBubbles } = loadInterruptionHelpers();
    const history = new FakeHistoryContainer();
    const live = new FakeChatMessage(
      ...
      ['read_file · 完成', 'sample_tool · 完成', '正在整理当前轮工具上下文'],
    );
    history.append(live);
    placeholders.set(key('cid-video', 'sample-agent', 'turn-that-kept-running'), live);
    ...
    if (!claimed && !deferHistory('cid-video', falseInterruption)) {
      history.append(new FakeChatMessage(
        ['chat-message', 'assistant'],
        { fromActor: 'sample-agent', systemKind: 'reply_interrupted' },
      ));
    }
  ...
  it('collapses an interruption loaded from an older page against a newer same-actor bubble', () => {
    ...
    const resumed = new FakeChatMessage(
      ['chat-message', 'assistant'],
      { fromActor: 'sample-agent', msgId: 'resumed-answer', turnId: 'turn-2' },
    );
    ...
    const olderInterruption = new FakeChatMessage(
      ['chat-message', 'assistant'],
      { fromActor: 'sample-agent', msgId: 'old-status', systemKind: 'reply_interrupted' },
    );
  ...
  it('preserves a genuine prior-turn interruption when a user message starts the resumed turn', () => {
    ...
    const interrupted = new FakeChatMessage(
      ['chat-message', 'assistant'],
      { fromActor: 'sample-agent', msgId: 'prior-status', systemKind: 'reply_interrupted' },
    );
    ...
    const resumed = new FakeChatMessage(
      ['chat-message', 'assistant'],
      { fromActor: 'sample-agent', msgId: 'next-answer', turnId: 'turn-next' },
    );
