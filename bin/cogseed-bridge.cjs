#!/usr/bin/env node
/**
 * cogseed-bridge — stdio MCP server an external CLI agent (claude code /
 * codex) spawns to reach the CogSeed environment (plan §D).
 *
...
function _connect() {
  ...
  _socket.on('close', () => failAll('bridge socket closed (CogSeed run may have ended)'));
  return _socket;
}
