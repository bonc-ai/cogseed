...
:args_done
if defined ORKAS_RUNTIME_VARIANT if not "%ORKAS_RUNTIME_VARIANT%"=="messaging" (
  echo [Mate Agent] This worktree is locked to the messaging runtime; ORKAS_RUNTIME_VARIANT=%ORKAS_RUNTIME_VARIANT% is not allowed. 1>&2
  exit /b 2
)
if defined ORKAS_WORKSPACE_ROOT (
  echo [Mate Agent] This worktree manages its own messaging data root; inherited ORKAS_WORKSPACE_ROOT is not allowed. 1>&2
  exit /b 2
)
set "ORKAS_RUNTIME_VARIANT=messaging"
...
:usage_ok
echo Usage: run.cmd
echo This worktree is locked to the messaging runtime identity.
exit /b 0
