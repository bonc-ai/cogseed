...
- [ ] T101 组详情按三盒分区渲染：isRelation 或值含 → 的字段归「规则」区，其余归「事实」区，区块标题四语
- [ ] T102 字段说明（description）显示——T 盒定义可视化
- [ ] T103 添加值识别 `A → B` 形状并提示「将作为规则保存」；新建字段可声明关系字段
- [ ] T104 vm 测试：分区断言 / 关系值写入后世界模型 ontologyRules 命中 / description 渲染

## Phase 2: 三盒总览 + 核实两档 + 来源修复包

- [ ] T201 「三盒总览」视图：A 盒合并（组字段值 + personal 资产）、R 盒合并（关系值 + 规则资产）、T 盒词汇统计，条目带来源侧标记
- [ ] T202 核实两档：FieldValue.verified 枚举化（true / 'independent'）、落盘 `@verified:independent`、IPC 透传、界面两枚章、世界模型档位
- [ ] T203 即时直投改传真实会话 id；无真实会话场景哈希 id + degraded 如实标注
- [ ] T204 渲染层剥 `immediate-` 前缀比对；证据统计按 id 去重
- [ ] T205 文案两分：「模型当场记（无独立出处）」≠「来源已删除」；「来源已删除的候选」分组收窄为真删除情形
- [ ] T206 测试：两档解析往返 / 总览合并计数 / 去重统计 / 剥前缀命中 / 新候选无合成 id；真机验证 7 条候选归组恢复
